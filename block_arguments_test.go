...
`
	...
	t.Run("passing lambda as parameter", func(t *testing.T) {
		source := `
apply := (f, x) -> f(x)
double := x -> x * 2
...
`
	...
	t.Run("block literal as argument", func(t *testing.T) {
		source := `
apply := f -> f()
...
// TestHigherOrderFunctions covers calling a function passed as a parameter,
// including the spec's apply_twice example (which previously mis-inlined the
// function parameter) and a non-inlinable variant that exercises the runtime
// indirect-call path.
func TestHigherOrderFunctions(t *testing.T) {
	t.Run("apply_twice (inlinable)", func(t *testing.T) {
		source := `
...
`
		output := compileAndRun(t, source)
		if !strings.Contains(output, "Result: 12") {
			t.Errorf("Expected Result: 12, got: %s", output)
		}
	})

	t.Run("apply_twice (non-inlinable block body)", func(t *testing.T) {
		source := `
apply := (f, x) -> { y := f(x); f(y) }
inc := x -> x + 1
printf("Result: %f\n", apply(inc, 10))
`
	...
	t.Run("inline lambda argument", func(t *testing.T) {
		source := `
apply := (f, x) -> f(x)
printf("Result: %f\n", apply(y -> y * 3, 7))
`
		output := compileAndRun(t, source)
		if !strings.Contains(output, "Result: 21") {
			t.Errorf("Expected Result: 21, got: %s", output)
		}
	})
}
