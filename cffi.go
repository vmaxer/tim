...
// sourceDir is the directory of the source file being compiled; used as an extra search root.
func ExtractConstantsFromLibrary(libName string, sourceDir string) (*CHeaderConstants, error) {
	constants := NewCHeaderConstants()

	// Get include paths from pkg-config
	includePaths, err := getPkgConfigIncludes(libName, sourceDir)
	if err != nil {
		if VerboseMode {
			fmt.Fprintf(os.Stderr, "Warning: pkg-config not available for %s: %v\n", libName, err)
		}
		// Fallback to standard paths including local include directories
		includePaths = buildFallbackIncludePaths(sourceDir)
	}
...
func getPkgConfigIncludes(libName string, sourceDir string) ([]string, error) {
	...
	for _, pkgName := range pkgNames {
		...
		var includes []string
		flags := strings.FieldsSeq(string(output))
	...
	// If pkg-config succeeded but no -I flags found, use fallback paths
	if pkgConfigSucceeded {
		if VerboseMode {
			fmt.Fprintf(os.Stderr, "No -I flags from pkg-config, using standard paths\n")
		}
		return buildFallbackIncludePaths(sourceDir), nil
	}

	return nil, lastErr
}

// buildFallbackIncludePaths returns a list of include search paths to try when pkg-config
// is unavailable or returns no -I flags. Searches relative to sourceDir and CWD.
func buildFallbackIncludePaths(sourceDir string) []string {
	...
	// Source-file-relative include dirs
	for _, rel := range []string{"include", ".", ".."} {
		add(filepath.Join(sourceDir, rel))
		add(filepath.Join(sourceDir, "..", rel))
	}

	// CWD-relative
	if cwd, err := os.Getwd(); err == nil {
		for _, rel := range []string{"include", "examples/include", "."} {
...
func parseHeaderFileWithDepth(headerPath string, constants *CHeaderConstants, visited map[string]bool, depth int) error {
	...
	maps.Copy(constants.Constants, parsedResults.Constants)
	maps.Copy(constants.Macros, parsedResults.Macros)
	maps.Copy(constants.Functions, parsedResults.Functions)
...
func parseIncludesWithDepth(headerPath string, constants *CHeaderConstants, visited map[string]bool, depth int) error {
	...
	// Build include search paths: header's own directory and standard system paths.
	// For library includes like <SDL3/SDL_init.h>, the parent of headerDir (e.g. the
	// "include/" root) covers sibling sub-directories.
	includeSearchPaths := []string{
		headerDir,
		filepath.Dir(headerDir), // one level up (e.g. examples/include when headerDir=examples/include/SDL3)
		"/usr/include",
		"/usr/local/include",
	}
	...
	for scanner.Scan() {
		...
		if includeMatches := includeRegex.FindStringSubmatch(line); len(includeMatches) == 2 {
			...
			for _, searchPath := range includeSearchPaths {
				testPath := filepath.Join(searchPath, includedFile)
...
func DiscoverFunctionSignatures(libraryName string, soPath string) (map[string]*CFunctionSignature, error) {
	...
	maps.Copy(signatures, headerSigs)
