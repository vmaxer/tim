...
func main() {
	fmt.Println("=== Tim C FFI Manager Demo ===\n")
...
`
	...
	for name, fn := range cffi.functions {
		returnType := cffi.MapCTypeToTim(fn.ReturnType)
		fmt.Printf("  %s: ", name)
		if fn.Library != "" {
		...
		for i, param := range fn.Params {
			timType := cffi.MapCTypeToTim(param.Type)
			if i > 0 {
				fmt.Print(", ")
			}
			fmt.Printf("%s: %s->%s", param.Name, param.Type, timType)
		}
		fmt.Println(")")
	}

	// Try to generate Tim bindings
	fmt.Println("\nGenerated Tim bindings:")
	for name := range cffi.functions {
		if binding, err := cffi.GenerateTimBinding(name); err == nil {
			fmt.Printf("\n%s\n", binding)
		}
	}

	// Show type mappings
	fmt.Println("\n\nC to Tim type mappings:")
	testTypes := []string{
		"int", "unsigned int", "char*", "const char*", "void*",
		"int64_t", "uint64_t", "float", "double",
	}
	for _, cType := range testTypes {
		timType := cffi.MapCTypeToTim(cType)
		fmt.Printf("  %-20s -> %s\n", cType, timType)
	}
