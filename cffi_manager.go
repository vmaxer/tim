...
// getDefaultTypeMappings returns the default C-to-Tim type mappings
...
func (cm *CFFIManager) ParseHeader(filepath string) error {
	...
	// Merge constants
	maps.Copy(cm.headerConstants.Constants, constants.Constants)

	// Merge macros
	maps.Copy(cm.headerConstants.Macros, constants.Macros)
...
// MapCTypeToTim maps a C type to a Tim type
func (cm *CFFIManager) MapCTypeToTim(cType string) string {
	// Normalize the type
	cType = strings.TrimSpace(cType)

	// Direct mapping
	if timType, ok := cm.typeMappings[cType]; ok {
		return timType
	}

	// Pointer types
	if before, ok := strings.CutSuffix(cType, "*"); ok {
...
// GenerateTimBinding generates a Tim function binding for a C function
func (cm *CFFIManager) GenerateTimBinding(funcName string) (string, error) {
	fn, ok := cm.functions[funcName]
	if !ok {
		return "", fmt.Errorf("function %s not found", funcName)
	}

	// Generate Tim function signature
	var paramTypes []string
	var paramNames []string
	for i, param := range fn.Params {
		timType := cm.MapCTypeToTim(param.Type)
		paramTypes = append(paramTypes, timType)
		if param.Name != "" {
	...
	returnType := cm.MapCTypeToTim(fn.ReturnType)
...
func (cm *CFFIManager) AutoLoadLibrary(libName string) error {
	...
	dllPaths := []string{
		...
		fmt.Sprintf("/usr/lib64/lib%s.so", libName),
		fmt.Sprintf("/usr/lib/lib%s.so", libName),
		fmt.Sprintf("/usr/local/lib64/lib%s.so", libName),
