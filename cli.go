...
func isTimFile(filename string) bool {
	return strings.HasSuffix(filename, ".tim")
}
...
func RunCLI(args []string, platform Platform, verbose, quiet bool, optTimeout float64, updateDeps, singleFile bool, outputPath string, depsOnly bool) error {
	...
	// If first arg is a .tim file and it starts with #!, we're in shebang mode
	if len(args) > 0 && isTimFile(args[0]) {
	...
	default:
		// Check if it's a .tim file (shorthand for build)
		if isTimFile(subcmd) {
			return cmdBuild(ctx, args)
		}

		// Check if it's a directory (compile all .tim files)
...
// cmdBuild compiles a Tim source file to an executable
...
func cmdBuild(ctx *CommandContext, args []string) error {
	...
	if outputPath == "" {
		...
		// Remove .tim extension
		baseName = strings.TrimSuffix(baseName, ".tim")
		baseName = strings.TrimSuffix(baseName, ".tim")
		outputPath = baseName
	...
	var err error
	if len(inputFiles) == 1 {
		err = CompileTimWithOptions(inputFiles[0], outputPath, ctx.Platform, ctx.OptTimeout, ctx.Verbose, ctx.DepsOnly)
	} else {
		...
		tmpFile, tmpErr := os.CreateTemp("", "tim_multi_*.tim")
		if tmpErr != nil {
		...
		err = CompileTimWithOptions(tmpPath, outputPath, ctx.Platform, ctx.OptTimeout, ctx.Verbose, ctx.DepsOnly)
	}
...
func cmdRun(ctx *CommandContext, args []string) error {
	...
	baseName := strings.TrimSuffix(filepath.Base(inputFile), ".tim")
	tmpExec := filepath.Join(tmpDir, fmt.Sprintf("tim_run_%s_%d", baseName, os.Getpid()))
...
func cmdRunShebang(ctx *CommandContext, scriptPath string, scriptArgs []string) error {
	...
	baseName := strings.TrimSuffix(filepath.Base(scriptPath), ".tim")
	tmpExec := filepath.Join(tmpDir, fmt.Sprintf("tim_shebang_%s_%d", baseName, os.Getpid()))
...
// cmdBuildDir finds the main .tim file in a directory and compiles it
// (does not compile test files or library files)
func cmdBuildDir(ctx *CommandContext, dirPath string) error {
	matches, err := filepath.Glob(filepath.Join(dirPath, "*.tim"))
	if err != nil {
		return fmt.Errorf("failed to find .tim files: %v", err)
	}
	...
	if len(nonTestFiles) == 0 {
		return fmt.Errorf("no non-test .tim files found in %s", dirPath)
	}
	...
	if mainFile == "" {
		return fmt.Errorf("no main function found in .tim files in %s", dirPath)
	}

	// Compile the main file
	outputPath := strings.TrimSuffix(filepath.Base(mainFile), ".tim")
	if ctx.Platform.OS == OSWindows {
		outputPath += ".exe"
	}
	...
	err = CompileTimWithOptions(mainFile, outputPath, ctx.Platform, ctx.OptTimeout, ctx.Verbose, ctx.DepsOnly)
	if err != nil {
...
// cmdTest runs all test_*.tim and *_test.tim files in the current directory
func cmdTest(ctx *CommandContext, args []string) error {
	...
	// Find all test files: test_*.tim and *_test.tim
	matchesPrefix, err := filepath.Glob(filepath.Join(searchDir, "test_*.tim"))
	if err != nil {
		return fmt.Errorf("failed to find test files: %v", err)
	}

	matchesSuffix, err := filepath.Glob(filepath.Join(searchDir, "*_test.tim"))
	if err != nil {
		return fmt.Errorf("failed to find test files: %v", err)
	}
	...
	for _, testFile := range matches {
		...
		baseName := strings.TrimSuffix(testName, ".tim")
		tmpExec := filepath.Join(tmpDir, fmt.Sprintf("tim_test_%s_%d", baseName, os.Getpid()))

		// Generate a test runner in the same directory as the test file for proper imports
		testDir := filepath.Dir(testFile)
		testRunnerPath := filepath.Join(testDir, fmt.Sprintf("_test_runner_%d.tim", os.Getpid()))
		...
		err = CompileTimWithOptions(testRunnerPath, tmpExec, ctx.Platform, ctx.OptTimeout, false, ctx.DepsOnly)
...
COMMANDS:
    ...
    run <file.tim>        Compile and run a Tim program immediately
    test [directory]      Run all test_*.tim files (default: current directory)
...
func generateTestRunner(runnerPath, testFile string, testFunctions []string) error {
	...
	testLines := strings.SplitSeq(string(testContent), "\n")
