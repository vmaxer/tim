...
//
// This file contains the TimCompiler and all code generation logic:
...
// Code Generator for Tim
type TimCompiler struct {
	...
	functionNamespace map[string]string  // function name -> namespace (for imported Tim functions)
...
func (fc *TimCompiler) nextLabel() string {
	fc.labelCounter++
	return fmt.Sprintf("L%d", fc.labelCounter)
}

// defineLabel marks a label at the current position
func (fc *TimCompiler) defineLabel(label string, pos int) {
	// Labels are tracked in ExecutableBuilder
	fc.eb.labels[label] = pos
}

func NewTimCompiler(platform Platform, verbose bool) (*TimCompiler, error) {
...
func (fc *TimCompiler) addSemanticError(message string, suggestions ...string) {
	suggestion := ""
...
// based on the target platform's calling convention. Delegates to cc().GetIntegerArgReg.
func (fc *TimCompiler) getIntArgReg(argIndex int) string {
	return fc.argReg(argIndex)
}

// allocateShadowSpace allocates the shadow space required by the calling convention.
// Returns the amount of space allocated (32 for Windows, 0 for other platforms).
func (fc *TimCompiler) allocateShadowSpace() int {
	size := fc.shadowSize()
	if size > 0 {
		fc.out.SubImmFromReg("rsp", int64(size))
	}
	return size
}

// deallocateShadowSpace removes the shadow space after a function call.
func (fc *TimCompiler) deallocateShadowSpace(shadowSpace int) {
	if shadowSpace > 0 {
...
func (fc *TimCompiler) allocateMemoryPlatform(sizeReg string) {
	if fc.eb.target.OS() == OSWindows {
		...
		// Preserve sizeReg when needed, but never clobber mmap return value in rax.
		// If size is in rax, copy it to r11 before issuing the syscall.
		if sizeReg == "rax" {
		...
		if sizeReg == "rax" {
			fc.out.MovRegToReg("rsi", "r11") // length = size
		} else {
		...
		// Restore size register if we saved it (never restore rax; it holds result).
		if sizeReg != "rsi" && sizeReg != "rax" {
...
func (fc *TimCompiler) processCImports(program *Program) {
		...
		if cImport, ok := stmt.(*CImportStmt); ok {
			...
			if cImport.SoPath == "" {
				...
				if ldOutput, ldErr := ldconfigCmd.Output(); ldErr == nil {
					lines := strings.SplitSeq(string(ldOutput), "\n")
			...
			if cImport.SoPath != "" {
				...
				if err != nil {
					...
					maps.Copy(fc.cConstants[cImport.Alias].Functions, signatures)
					if VerboseMode {
			...
			constants, err := ExtractConstantsFromLibrary(cImport.Library, fc.sourceDir)
			if err != nil {
				...
				maps.Copy(fc.cConstants[cImport.Alias].Constants, constants.Constants)
				maps.Copy(fc.cConstants[cImport.Alias].Macros, constants.Macros)
...
func (fc *TimCompiler) detectMainCallInTopLevel(statements []Statement) bool {
	return slices.ContainsFunc(statements, fc.statementCallsMain)
}

// statementCallsMain recursively checks if a statement calls main()
func (fc *TimCompiler) statementCallsMain(stmt Statement) bool {
	switch s := stmt.(type) {
	case *ExpressionStmt:
	...
	case *IfStmt:
		for _, branch := range s.Branches {
			if fc.expressionCallsMain(branch.Condition) {
				return true
			}
			if slices.ContainsFunc(branch.Body, fc.statementCallsMain) {
				return true
			}
		}
		return slices.ContainsFunc(s.ElseBody, fc.statementCallsMain)
	default:
...
func (fc *TimCompiler) expressionCallsMain(expr Expression) bool {
	if expr == nil {
		return false
	}
	...
	case *CallExpr:
		...
		return slices.ContainsFunc(e.Args, fc.expressionCallsMain)
	...
	case *ListExpr:
		return slices.ContainsFunc(e.Elements, fc.expressionCallsMain)
	...
	case *BlockExpr:
		...
		return slices.ContainsFunc(e.Statements, fc.statementCallsMain)
...
func (fc *TimCompiler) trackDependenciesInStatement(stmt Statement) {
	switch s := stmt.(type) {
	case *AssignStmt:
	...
	case *IfStmt:
		for _, branch := range s.Branches {
			fc.trackDependenciesInExpr(branch.Condition)
			for _, bodyStmt := range branch.Body {
				fc.trackDependenciesInStatement(bodyStmt)
			}
		}
...
func (fc *TimCompiler) trackDependenciesInExpr(expr Expression) {
	if expr == nil {
		return
	}
...
func (fc *TimCompiler) collectAllFunctions(program *Program) {
	if VerboseMode {
...
func (fc *TimCompiler) reorderStatementsForForwardRefs(statements []Statement) []Statement {
...
func (fc *TimCompiler) Compile(program *Program, outputPath string) error {
	...
	if VerboseMode {
		debugf("DEBUG: mainCalledAtTopLevel = %v\n", fc.mainCalledAtTopLevel)
	}
	...
	if fc.debug {
		if VerboseMode {
			debugf("DEBUG Compile: starting compilation with %d statements\n", len(program.Statements))
		}
	}
...
func (fc *TimCompiler) CompileDepsOnly(program *Program) error {
...
func (fc *TimCompiler) compileInternal(program *Program, outputPath string, depsOnly bool) error {
	// Clear moved variables tracking for this compilation
	fc.movedVars = make(map[string]bool)
	fc.scopedMoved = []map[string]bool{make(map[string]bool)}
	...
	// Copy cstruct declarations from program for field access codegen
	maps.Copy(fc.cstructs, program.CStructs)
	...
	// Always define CPU feature flags (initialized to 0 = not supported)
	// On non-Windows, CPUID detection below will update these at startup
	...
	for i, stmt := range program.Statements {
		if VerboseMode && i < 10 {
			if assign, ok := stmt.(*AssignStmt); ok {
				debugf("DEBUG collectSymbols [%d]: %s (Mutable=%v, IsUpdate=%v)\n",
	...
	if fc.usesArenas {
		fc.eb.DefineWritable("_tim_arena_meta", "\x00\x00\x00\x00\x00\x00\x00\x00")     // Pointer to arena array
		fc.eb.DefineWritable("_tim_arena_meta_cap", "\x00\x00\x00\x00\x00\x00\x00\x00") // Capacity (number of slots)
		fc.eb.DefineWritable("_tim_arena_meta_len", "\x00\x00\x00\x00\x00\x00\x00\x00") // Length (number of active arenas)
	...
	// currentArena is already set to 1 in NewTimCompiler (representing meta-arena[0])
	// Arena initialization will be added later if needed (tracked by usesArenas flag)
	// We'll patch in a call to _tim_init_arenas before main if arenas are used
	...
	if VerboseMode {
		debugf("DEBUG: Compiling %d statements\n", len(program.Statements))
		for i, stmt := range program.Statements {
			debugf("DEBUG:   Statement %d: %T\n", i, stmt)
		}
	}
	for i, stmt := range program.Statements {
		if VerboseMode {
			debugf("DEBUG: About to compile statement %d: %T\n", i, stmt)
		}
		fc.compileStatement(stmt)
		if VerboseMode {
			debugf("DEBUG: Finished compiling statement %d\n", i)
		}
	}
	...
	if fc.functionSignatures["main"] != nil {
	...
	if fc.eb.target.IsPE() {
		if VerboseMode {
			debugf("DEBUG: Generating runtime helpers for PE (before cleanup)\n")
		}
...
func (fc *TimCompiler) updateStackOffset(delta int) {
...
func (fc *TimCompiler) collectSymbols(stmt Statement) error {
	switch s := stmt.(type) {
	case *AssignStmt:
		...
		if VerboseMode {
			debugf("DEBUG collectSymbols AssignStmt: name=%s, exists=%v, IsUpdate=%v, Mutable=%v, mutableVars[%s]=%v, stackOffset=%d\n",
		...
		if s.Name == "r1" && exists {
			debugf("DEBUG: Variable 'r1' already exists! Stack trace:\n")
			debug.PrintStack()
		}
		...
		if s.IsUpdate {
			...
			// Allocate storage: globals in .data, locals on stack
			if fc.collectingSymbols && fc.currentLambda == nil {
				fc.moduleLevelVars[s.Name] = true
				dataOffset := len(fc.dataSection)
				fc.dataSection = append(fc.dataSection, make([]byte, 8)...)
				fc.globalVars[s.Name] = dataOffset
				fc.globalVarsMutable[s.Name] = true
				fc.variables[s.Name] = -1
			...
			if fc.debug {
				if VerboseMode {
					if fc.currentLambda == nil {
						debugf("DEBUG collectSymbols: storing mutable global variable '%s' at data offset %d\n", s.Name, fc.globalVars[s.Name])
					} else {
						debugf("DEBUG collectSymbols: storing mutable variable '%s' at offset %d\n", s.Name, fc.variables[s.Name])
					}
				}
			}
			...
			if s.TypeAnnotation != nil {
				...
				if VerboseMode {
					debugf("DEBUG: Setting varTypeInfo[%s] = %s (mutable, annotated)\n", s.Name, s.TypeAnnotation.String())
				}
			}

			// Track cstruct type for field access codegen
			if castExpr, ok := s.Value.(*CastExpr); ok {
				if _, isCStruct := fc.cstructs[castExpr.Type]; isCStruct {
			...
			if exprType != "number" && exprType != "unknown" {
				...
				if VerboseMode {
					debugf("DEBUG: Setting varTypes[%s] = %s (mutable)\n", s.Name, exprType)
				}
			}
		} else {
			...
			if exists && fc.mutableVars[s.Name] {
				...
				if fc.debug {
					if VerboseMode {
						if fc.currentLambda == nil {
							debugf("DEBUG collectSymbols: storing immutable global variable '%s' at data offset %d\n", s.Name, fc.globalVars[s.Name])
						} else {
							debugf("DEBUG collectSymbols: storing immutable variable '%s' at offset %d\n", s.Name, fc.variables[s.Name])
						}
					}
				}
				...
				if s.TypeAnnotation != nil {
					fc.varTypeInfo[s.Name] = s.TypeAnnotation
					if VerboseMode {
						debugf("DEBUG: Setting varTypeInfo[%s] = %s (immutable, annotated)\n", s.Name, s.TypeAnnotation.String())
					}
				}
				...
				exprType := fc.getExprType(s.Value)
				if fc.debug {
					debugf("DEBUG TYPE TRACKING: var=%s, exprType=%s, Value type=%T\n", s.Name, exprType, s.Value)
				}
				if exprType != "number" && exprType != "unknown" {
					fc.varTypes[s.Name] = exprType
					if VerboseMode {
						debugf("DEBUG: Setting varTypes[%s] = %s (immutable)\n", s.Name, exprType)
					}
				}
			}
		}
	...
	case *LoopStmt:
		...
		if s.BaseOffset == 0 {
			...
			if VerboseMode {
				debugf("DEBUG collectSymbols: Storing baseOffset=%d in LoopStmt (first pass)\n", baseOffset)
			}
		} else {
			if VerboseMode {
				debugf("DEBUG collectSymbols: BaseOffset already set to %d, not overwriting with %d\n",
					s.BaseOffset, baseOffset)
			}
		}
	...
	case *ArenaStmt:
		...
		// Note: Arena pointers are stored in static storage (_tim_arena_ptrs)
		for _, bodyStmt := range s.Body {
			if err := fc.collectSymbols(bodyStmt); err != nil {
				return err
			}
...
// Confidence that this function is working: 100%
func (fc *TimCompiler) compileStatement(stmt Statement) {
	switch s := stmt.(type) {
	case *AssignStmt:
		if fc.debug {
			if VerboseMode {
				debugf("DEBUG compileStatement: compiling assignment '%s' (type: %T)\n", s.Name, s.Value)
			}
		}
...
func (fc *TimCompiler) pushDeferScope() {
	if VerboseMode {
		debugf("DEBUG: pushDeferScope called, len before = %d\n", len(fc.deferredExprs))
	}
	fc.deferredExprs = append(fc.deferredExprs, []Expression{})
	if VerboseMode {
		debugf("DEBUG: pushDeferScope called, len after = %d\n", len(fc.deferredExprs))
	}
}

func (fc *TimCompiler) popDeferScope() {
	if VerboseMode {
		debugf("DEBUG: popDeferScope called, len before = %d\n", len(fc.deferredExprs))
	}
	if len(fc.deferredExprs) == 0 {
		return
	}
	...
	if VerboseMode {
		debugf("DEBUG: popDeferScope done, len after = %d\n", len(fc.deferredExprs))
	}
}

func (fc *TimCompiler) compileArenaStmt(stmt *ArenaStmt) {
	...
	// Call _tim_arena_ensure_capacity(arenaIndex + 1)
	if fc.eb.target.OS() == OSWindows {
		fc.out.MovImmToReg("rcx", fmt.Sprintf("%d", fc.currentArena))
		shadowSpace := fc.allocateShadowSpace()
		fc.out.CallSymbol("_tim_arena_ensure_capacity")
		fc.deallocateShadowSpace(shadowSpace)
	} else {
	...
	// Load arena pointer from meta-arena: _tim_arena_meta[arenaIndex]
	// Each pointer is 8 bytes, so offset = arenaIndex * 8
	offset := arenaIndex * 8
	fc.out.LeaSymbolToReg("rax", "_tim_arena_meta")
...
func (fc *TimCompiler) compileArenaExpr(expr *ArenaExpr) {
	// Mark that this program uses arenas
	fc.usesArenas = true
...
func (fc *TimCompiler) compileWhileStatement(stmt *WhileStmt) {
	...
	fc.activeLoops[len(fc.activeLoops)-1].EndPatches = append(
		fc.activeLoops[len(fc.activeLoops)-1].EndPatches, conditionJumpPos+2)
	...
	// Patch all continue jumps to point to the increment/back-jump
	continuePos := fc.eb.text.Len()
	for _, patchPos := range fc.activeLoops[len(fc.activeLoops)-1].ContinuePatches {
		offset := int32(continuePos - (patchPos + 4))
		fc.patchJumpImmediate(patchPos, offset)
	}
	...
	for _, patchPos := range fc.activeLoops[len(fc.activeLoops)-1].EndPatches {
		offset := int32(loopEndPos - (patchPos + 4)) // patchPos points at 4-byte offset field
		fc.patchJumpImmediate(patchPos, offset)
	}
...
func (fc *TimCompiler) compileIfStatement(stmt *IfStmt) {
	endJumps := []int{}

	for _, branch := range stmt.Branches {
		// Evaluate branch condition and skip branch when false (0.0).
		fc.compileExpression(branch.Condition)
		fc.out.XorpdXmm("xmm1", "xmm1")
		fc.out.Ucomisd("xmm0", "xmm1")
		skipBranchJumpPos := fc.eb.text.Len()
		fc.out.JumpConditional(JumpEqual, 0)
		skipBranchJumpEnd := fc.eb.text.Len()

		for _, bodyStmt := range branch.Body {
			fc.compileStatement(bodyStmt)
		}

		// Branch matched; jump to the end of the full conditional chain.
		endJumpPos := fc.eb.text.Len()
		fc.out.JumpUnconditional(0)
		endJumps = append(endJumps, endJumpPos)

		nextBranchPos := fc.eb.text.Len()
		fc.patchJumpImmediate(skipBranchJumpPos+2, int32(nextBranchPos-skipBranchJumpEnd))
	}
	...
	for _, jumpPos := range endJumps {
		fc.patchJumpImmediate(jumpPos+1, int32(endPos-(jumpPos+UnconditionalJumpSize)))
	}
}
...
func (fc *TimCompiler) compileRangeLoop(stmt *LoopStmt, rangeExpr *RangeExpr) {
	...
	if VerboseMode {
		debugf("DEBUG compileRangeLoop: currentLoopLabel=%d, baseOffset=%d from stmt.BaseOffset\n",
	...
	if VerboseMode {
		debugf("DEBUG compileRangeLoop: depth=%d, counterReg='%s', useRegister=%v\n",
	...
	if VerboseMode {
		debugf("DEBUG: Loop iterator '%s' at offset %d (baseOffset=%d, loopStateOffset=%d)\n",
			stmt.Iterator, iterOffset, baseOffset, loopStateOffset)
		debugf("DEBUG: Current variables before iterator: %v\n", fc.variables)
	}
...
func (fc *TimCompiler) compileParallelRangeLoop(stmt *LoopStmt, rangeExpr *RangeExpr) {
	...
	if VerboseMode {
		debugf("DEBUG: Compiling parallel range loop with %d threads, iterator '%s'\n",
	...
	if totalItems <= 0 {
		...
		if VerboseMode {
			debugf("DEBUG: Skipping parallel loop with empty range [%d, %d)\n", start, end)
		}
	...
	if VerboseMode {
		debugf("DEBUG: Range [%d, %d) = %d items\n", start, end, totalItems)
		debugf("DEBUG: Each thread: ~%d items (remainder: %d)\n", chunkSize, remainder)
	}
...
func (fc *TimCompiler) compileListLoop(stmt *LoopStmt) {
	fc.labelCounter++
...
func (fc *TimCompiler) compileJumpStatement(stmt *JumpStmt) {
...
func (fc *TimCompiler) patchJumpImmediate(pos int, offset int32) {
	...
	if fc.debug {
		if VerboseMode {
			debugf("DEBUG PATCH: Before patching at pos %d: %02x %02x %02x %02x\n", pos, bytes[pos], bytes[pos+1], bytes[pos+2], bytes[pos+3])
		}
	}
	...
	case *CallExpr:
		...
		if strings.Contains(e.Function, ".") {
			...
			if len(parts) == 2 {
				...
				// Only treat as C FFI if the namespace is a registered C import
				if fc.cImports[alias] != "" {
					...
					// Default for unknown C FFI: assume number
					return "number"
				}
				// Not a C import - fall through to method call handler below
				_ = funcName
			}
		}

		// Function calls - check return type for Tim built-ins
		// For method call syntax (xs.append), extract just the method name
		funcName := e.Function
		if strings.Contains(funcName, ".") {
			parts := strings.Split(funcName, ".")
			if len(parts) == 2 && fc.cImports[parts[0]] == "" {
				funcName = parts[1]
			}
		}
...
// ptrReg must hold the pointer (as an integer, not a float).
func (fc *TimCompiler) emitCStructFieldRead(ptrReg string, field *CStructField) {
	ptrEnc := uint8(7) // rdi default
	if r, ok := GetRegister(fc.out.target.Arch(), ptrReg); ok {
		ptrEnc = uint8(r.Encoding & 7)
	}

	emitModRM := func(regField, mod uint8) {
		if mod == 0 && ptrEnc != 5 { // no disp if not rbp
			fc.out.Write(mod<<6 | regField<<3 | ptrEnc)
			if ptrEnc == 4 { // rsp/r12 needs SIB
				fc.out.Write(0x24)
			}
		} else if field.Offset < 128 {
			fc.out.Write(0x40 | regField<<3 | ptrEnc)
			if ptrEnc == 4 {
				fc.out.Write(0x24)
			}
			fc.out.Write(uint8(field.Offset))
		} else {
			fc.out.Write(0x80 | regField<<3 | ptrEnc)
			if ptrEnc == 4 {
				fc.out.Write(0x24)
			}
			offset := uint32(field.Offset)
			fc.out.Write(uint8(offset))
			fc.out.Write(uint8(offset >> 8))
			fc.out.Write(uint8(offset >> 16))
			fc.out.Write(uint8(offset >> 24))
		}
	}

	dispMod := uint8(0)
	if field.Offset != 0 || ptrEnc == 5 {
		if field.Offset < 128 {
			dispMod = 1
		} else {
			dispMod = 2
	...
	switch field.Type {
	case "uint8":
		// movzx eax, byte [ptrReg + offset]
		fc.out.Write(0x0F)
		fc.out.Write(0xB6)
		emitModRM(0, dispMod)
		fc.out.Cvtsi2sd("xmm0", "rax")
	case "int8":
		// movsx eax, byte [ptrReg + offset]
		fc.out.Write(0x0F)
		fc.out.Write(0xBE)
		emitModRM(0, dispMod)
		fc.out.Cvtsi2sd("xmm0", "rax")
	case "uint16":
		// movzx eax, word [ptrReg + offset]
		fc.out.Write(0x0F)
		fc.out.Write(0xB7)
		emitModRM(0, dispMod)
		fc.out.Cvtsi2sd("xmm0", "rax")
	case "int16":
		// movsx eax, word [ptrReg + offset]
		fc.out.Write(0x0F)
		fc.out.Write(0xBF)
		emitModRM(0, dispMod)
		fc.out.Cvtsi2sd("xmm0", "rax")
	case "uint32":
		// mov eax, [ptrReg + offset]  (no REX → zero-extends to rax)
		fc.out.Write(0x8B)
		emitModRM(0, dispMod)
		fc.out.Cvtsi2sd("xmm0", "rax")
	case "int32":
		// movsxd rax, [ptrReg + offset]
		fc.out.Write(0x48)
		fc.out.Write(0x63)
		emitModRM(0, dispMod)
		fc.out.Cvtsi2sd("xmm0", "rax")
	case "uint64", "int64", "ptr", "cstr":
		// mov rax, [ptrReg + offset]
		fc.out.Write(0x48)
		fc.out.Write(0x8B)
		emitModRM(0, dispMod)
		fc.out.Cvtsi2sd("xmm0", "rax")
	case "float32":
		// movss xmm0, [ptrReg + offset]
		fc.out.Write(0xF3)
		fc.out.Write(0x0F)
		fc.out.Write(0x10)
		emitModRM(0, dispMod)
...
// emitCStructFieldWrite emits code to write rax (integer) or xmm0 (float) into [ptrReg + offset].
// Before calling: integer value in rax for int/uint/ptr fields, xmm0 for float fields.
func (fc *TimCompiler) emitCStructFieldWrite(ptrReg string, field *CStructField) {
	ptrEnc := uint8(7) // rdi default
	if r, ok := GetRegister(fc.out.target.Arch(), ptrReg); ok {
		ptrEnc = uint8(r.Encoding & 7)
	}
	...
	switch field.Type {
	case "uint8", "int8":
		// mov byte [ptrReg + offset], al
		fc.out.Write(0x88)
		emitModRM(0, dispMod)
	case "uint16", "int16":
		// mov word [ptrReg + offset], ax  (66 89 /r)
		fc.out.Write(0x66)
		fc.out.Write(0x89)
		emitModRM(0, dispMod)
	case "uint32", "int32":
		// mov dword [ptrReg + offset], eax  (89 /r, no REX)
		fc.out.Write(0x89)
		emitModRM(0, dispMod)
	case "uint64", "int64", "ptr", "cstr":
		// mov qword [ptrReg + offset], rax  (48 89 /r)
		fc.out.Write(0x48)
		fc.out.Write(0x89)
		emitModRM(0, dispMod)
	case "float32":
		// movss [ptrReg + offset], xmm0
		fc.out.Write(0xF3)
		fc.out.Write(0x0F)
		fc.out.Write(0x11)
		emitModRM(0, dispMod)
	case "float64":
		// movsd [ptrReg + offset], xmm0
		fc.out.Write(0xF2)
		fc.out.Write(0x0F)
		fc.out.Write(0x11)
		emitModRM(0, dispMod)
...
func (fc *TimCompiler) compileExpression(expr Expression) {
	if expr == nil {
		debugf("DEBUG: nil expression stack trace:\n")
		debug.PrintStack()
		compilerError("INTERNAL ERROR: compileExpression received nil expression")
	}
	if VerboseMode {
		debugf("DEBUG compileExpression: expr type = %T\n", expr)
	}
	switch e := expr.(type) {
	case *NumberExpr:
		// Tim uses float64 foundation - all values are float64
	...
	case *StringExpr:
		...
		if fc.cContext {
			...
			// Tim context: compile as map[uint64]float64 where keys are indices
	...
	case *FStringExpr:
		...
		for i := 1; i < len(e.Parts); i++ {
			...
			// Call _tim_string_concat(rdi, rsi) -> rax
			fc.trackFunctionCall("_tim_string_concat")
			fc.out.CallSymbol("_tim_string_concat")
	...
	case *IdentExpr:
		...
		if dataOffset, isGlobal := fc.globalVars[e.Name]; isGlobal {
			...
			if !exists {
				if VerboseMode {
					debugf("DEBUG: Undefined variable '%s', available vars: %v\n", e.Name, fc.variables)
					debugf("DEBUG: Current lambda: %v\n", fc.currentLambda)
				}
	...
	case *FieldAccessExpr:
		// Check if this is a C struct field access
		if decl := fc.getCStructForExpr(e.Object); decl != nil {
			var field *CStructField
			...
			if field != nil {
				...
				fc.emitCStructFieldRead("rdi", field)
				break
			}
		}
		// Fallback: Tim map key lookup
	...
	case *BinaryExpr:
		...
		if e.Operator == "+" {
			...
			if leftType == "string" && rightType == "string" {
				...
				// Call _tim_string_concat(left_ptr, right_ptr)
			...
			if leftType == "list" && rightType == "list" {
				...
				// Call _tim_list_concat(left_ptr, right_ptr)
			...
			if leftType == "list" || leftType == "unknown" {
				...
				fc.out.PushReg("rcx") // rep movsq clobbers rcx; rax preserved

				fc.out.LeaMemToReg("rdi", "rax", 8)
				fc.out.LeaMemToReg("rsi", "rsi", 8)
				fc.out.ShlImmReg("rcx", 1) // rcx = old_count * 2 (qwords, each entry = 16 bytes)
		...
		if e.Operator == "==" || e.Operator == "!=" {
			...
			if leftType == "string" && rightType == "string" {
				...
				// Call _tim_string_eq(left_ptr, right_ptr)
				fc.out.MovMemToReg("rdi", "rsp", 16) // left ptr
				fc.out.MovMemToReg("rsi", "rsp", 0)  // right ptr
				fc.out.AddImmToReg("rsp", 32)
				...
				fc.trackFunctionCall("_tim_string_eq")
				fc.out.CallSymbol("_tim_string_eq")
		...
		case "mod", "%":
			...
			// Modulo by zero: return error NaN with "dv0\0" code (same semantics as division).
			fc.out.Emit([]byte{0x48, 0xb8})                                     // mov rax, immediate64
			fc.out.Emit([]byte{0x00, 0x30, 0x76, 0x64, 0x00, 0x00, 0xf8, 0x7f}) // NaN with "dv0\0"
			fc.out.SubImmFromReg("rsp", 8)
			fc.out.MovRegToMem("rax", "rsp", 0)
	...
	case *LambdaExpr:
		...
		if VerboseMode {
			debugf("DEBUG compileExpression: adding lambda '%s' with %d params, variadic='%s', body type: %T\n", funcName, len(e.Params), e.VariadicParam, e.Body)
		}
	...
	case *BlockExpr:
		...
		for i, stmt := range e.Statements {
			...
			if i == len(e.Statements)-1 {
				...
				if VerboseMode {
					debugf("DEBUG BlockExpr: last statement type = %T\n", stmt)
				}
				...
				if assignStmt, ok := stmt.(*AssignStmt); ok {
					...
					// Return 1.0 (true/success) implicitly
					if VerboseMode {
						debugf("DEBUG BlockExpr: last statement is NOT ExpressionStmt, returning 1.0\n")
					}
					fc.compileExpression(&NumberExpr{Value: 1.0})
				}
...
func (fc *TimCompiler) compileMatchExpr(expr *MatchExpr) {
	...
	endPos := fc.eb.text.Len()
	if fc.debug {
		debugf("DEBUG JUMP PATCHING: endPos = %d, patching %d jumps\n", endPos, len(endJumpPositions))
	}
	for i, jumpPos := range endJumpPositions {
		endOffset := int32(endPos - (jumpPos + 5))
		if fc.debug {
			debugf("DEBUG JUMP PATCHING: jump %d at pos %d -> endOffset %d (target pos %d)\n",
...
func (fc *TimCompiler) compileMatchClauseResult(result Expression, endJumps *[]int) {
	...
	jumpPos := fc.eb.text.Len()
	if fc.debug {
		debugf("DEBUG JUMP PATCHING: adding jump at pos %d to endJumps list (count before: %d)\n",
...
func (fc *TimCompiler) compileMatchDefault(result Expression) {
	if jumpExpr, isJump := result.(*JumpExpr); isJump {
		fc.compileMatchJump(jumpExpr)
		return
	}
...
func (fc *TimCompiler) compileMatchJump(jumpExpr *JumpExpr) {
...
func (fc *TimCompiler) compileCastExpr(expr *CastExpr) {
	if VerboseMode {
		debugf("DEBUG compileCastExpr: Type=%s, Expr type=%T\n", expr.Type, expr.Expr)
	}
	...
	case "number":
		// Convert C return value to Tim number (identity, already float64)
	...
	case "cstr":
		...
		// Convert Tim string to C null-terminated string
		// xmm0 contains pointer to Tim string map
		// Call runtime function: _tim_string_to_cstr(xmm0) -> rax
	...
	case "string", "str":
		...
		if exprType == "list" || exprType == "map" {
			...
			if VerboseMode {
				debugf("DEBUG: Converting %s to string placeholder\n", exprType)
			}
			placeholder := "[...]"
			if exprType == "map" {
				placeholder = "{...}"
		...
		// Convert number to string using _tim_itoa (pure machine code, no libc)
		...
		// Call _tim_itoa(rdi=number) -> (rsi=buffer, rdx=length)
		// Save r15 before call (it contains buffer pointer)
		fc.out.PushReg("r15")
		fc.callFunction("_tim_itoa", "")
		fc.out.PopReg("r15")

		// _tim_itoa returns: rsi=buffer start, rdx=length
		...
		// Load one byte from [rsi] into AL/zero-extended RAX.
		// MovMemToReg is 64-bit-only and cannot be used with byte regs like AL.
		fc.out.Emit([]byte{0x48, 0x0f, 0xb6, 0x06}) // movzx rax, byte [rsi]
		...
		// Convert C string to Tim string: allocate 8 + len*16 bytes
		...
		fc.out.MovRegToReg("r14", "rax") // r14 = Tim string

		// Store count
		fc.out.Cvtsi2sd("xmm0", "r13")
		fc.out.MovXmmToMem("xmm0", "r14", 0)

		// Copy characters to Tim string
		...
		// Store to Tim string at offset 8 + rcx*16
		fc.out.MovRegToReg("rdx", "rcx")
		...
		// Clean up buffer and return Tim string pointer in xmm0
		fc.out.AddImmToReg("rsp", 32)
...
func (fc *TimCompiler) compileSliceExpr(expr *SliceExpr) {
	...
	// Call runtime function: _tim_slice_string(collection_ptr, start, end, step) -> new_collection_ptr
	...
	fc.out.CallSymbol("_tim_slice_string")
...
func (fc *TimCompiler) compileUnsafeExpr(expr *UnsafeExpr) {
...
func (fc *TimCompiler) compileSyscall() {
...
func (fc *TimCompiler) compileRegisterAssignment(stmt *RegisterAssignStmt) {
...
func (fc *TimCompiler) compileRegisterOp(dest string, op *RegisterOp) {
	// Unary operations
	if op.Left == "" {
		if op.Operator == "~b" {
			// NOT: dest <- ~b src
...
func (fc *TimCompiler) compileMemoryLoad(dest string, load *MemoryLoad) {
...
func (fc *TimCompiler) compileSizedMemoryStore(store *MemoryStore) {
...
func (fc *TimCompiler) emitNullPointerCheck(reg string) {
...
func (fc *TimCompiler) compileUnsafeCast(dest string, cast *CastExpr) {
	...
	case *IdentExpr:
		...
		if offset, ok := fc.variables[expr.Name]; ok {
			...
			if cast.Type == "cstr" || cast.Type == "cstring" {
				// Convert Tim string to C null-terminated string
				// xmm0 contains pointer to Tim string map
				// _tim_string_to_cstr is an internal runtime function, not external
				fc.trackFunctionCall("_tim_string_to_cstr")
				fc.trackFunctionCall("_tim_string_to_cstr")
				fc.out.CallSymbol("_tim_string_to_cstr")
...
func (fc *TimCompiler) generateLambdaFunctions() {
	...
	for i := 0; i < len(fc.lambdaFuncs); i++ {
		...
		if VerboseMode {
			debugf("DEBUG generateLambdaFunctions: generating lambda '%s' with body type %T\n", lambda.Name, lambda.Body)
		}
		...
		if lambda.VariadicParam != "" {
			debugf("DEBUG: Lambda '%s' has variadic param '%s'\n", lambda.Name, lambda.VariadicParam)
			if VerboseMode {
				debugf("DEBUG generateLambdaFunctions: '%s' HAS variadic param '%s' (fixedParams=%d)\n", lambda.Name, lambda.VariadicParam, paramCount)
			}
			...
			// Allocate from arena: _tim_arena_alloc(arena_ptr, size)
			...
			maxVariadic := min(6-paramCount, 6)

			for i := range maxVariadic {
				xmmIdx := paramCount + i
				if xmmIdx >= 6 {
					break
				}
...
func (fc *TimCompiler) buildHotFunctionTable() {
...
func (fc *TimCompiler) generateHotFunctionTable() {
	if len(fc.hotFunctions) == 0 {
		return
	}
...
func (fc *TimCompiler) patchHotFunctionTable() {
	if len(fc.hotFunctions) == 0 {
		return
	}
...
func (fc *TimCompiler) generateRuntimeHelpers() {
	...
	if !fc.usesArenas && (fc.usedFunctions["_tim_string_concat"] || fc.usedFunctions["_tim_string_to_cstr"]) {
		fc.usesArenas = true
		...
		fc.eb.DefineWritable("_tim_arena_meta_len", "\x00\x00\x00\x00\x00\x00\x00\x00")
		fc.eb.Define("_arena_null_error", "ERROR: Arena alloc returned NULL\n\x00")
	}
	...
	if fc.usesArenas {
		...
		if fc.arenaInitCallOffset > 0 {
			// Get the address of _tim_init_arenas
			initFuncOffset, ok := fc.eb.labels["_tim_init_arenas"]
			if ok {
				...
				debugf("DEBUG: Patched arena init call at offset 0x%X to call 0x%X (disp=%d)\n",
					fc.arenaInitCallOffset, initFuncOffset, disp)
			} else {
				fmt.Fprintf(os.Stderr, "WARNING: Could not find _tim_init_arenas label\n")
			}
		}
	}

	// Arena runtime functions are generated inline below (_tim_arena_create, alloc, etc)
	...
	// Generate _tim_string_concat only if used
	if fc.usedFunctions["_tim_string_concat"] {
		if VerboseMode {
			debugf("DEBUG: Generating _tim_string_concat\n")
		}
		...
		fc.eb.MarkLabel("_tim_string_concat")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_string_concat used

	// Generate _tim_string_to_cstr only if explicitly used (for f-strings, C FFI)
	// Note: println/printf don't use this - they have their own inline implementations
	if fc.usedFunctions["_tim_string_to_cstr"] {
		if VerboseMode {
			debugf("DEBUG: Generating _tim_string_to_cstr\n")
		}
		// Generate _tim_string_to_cstr(tim_string_ptr) -> cstr_ptr
		// Converts a Tim string (map format) to a null-terminated C string
		// Argument: xmm0 = Tim string pointer (as float64)
		// Returns: rax = C string pointer
		fc.eb.MarkLabel("_tim_string_to_cstr")
		...
		// callArenaAlloc expects size in rdi (internal ABI, all platforms)
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_string_to_cstr used

	// Generate _tim_cstr_to_string only if used (for C FFI string returns)
	if fc.usedFunctions["_tim_cstr_to_string"] {
		// Generate _tim_cstr_to_string(cstr_ptr) -> tim_string_ptr
		// Converts a null-terminated C string to a Tim string (map format)
		// Argument: rdi = C string pointer
		// Returns: xmm0 = Tim string pointer (as float64)
		fc.eb.MarkLabel("_tim_cstr_to_string")
		...
		// Allocate from arena
		fc.callArenaAlloc()
		fc.out.MovRegToReg("r13", "rax") // r13 = Tim string map pointer
		...
		fc.eb.MarkLabel("_cstr_to_tim_loop")
		...
		// Return Tim string pointer in xmm0
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_cstr_to_string used

	// Generate _tim_slice_string only if used
	if fc.usedFunctions["_tim_slice_string"] {
		// Generate _tim_slice_string(str_ptr, start, end, step) -> new_str_ptr
		...
		fc.eb.MarkLabel("_tim_slice_string")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_slice_string used

	// Generate _tim_list_concat only if list operations are used
	...
	if fc.usedFunctions["_tim_list_concat"] {
		fc.eb.MarkLabel("_tim_list_concat")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_concat used

	// Generate _tim_list_repeat only if list repeat operations are used
	...
	if fc.usedFunctions["_tim_list_repeat"] {
		fc.eb.MarkLabel("_tim_list_repeat")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_repeat used

	// Generate _tim_string_eq only if string equality checks are used
	// Arguments: rdi = left_ptr, rsi = right_ptr
	// Returns: xmm0 = 1.0 if equal, 0.0 if not
	// String format: [count (8 bytes)][key0 (8)][val0 (8)][key1 (8)][val1 (8)]...
	if fc.usedFunctions["_tim_string_eq"] {
		fc.eb.MarkLabel("_tim_string_eq")
		...
		// Generate upper_string(tim_string_ptr) -> uppercase_tim_string_ptr
		// Converts a Tim string to uppercase
		// Argument: rdi = Tim string pointer (as integer)
		// Returns: xmm0 = uppercase Tim string pointer (as float64)
		...
		// Generate lower_string(tim_string_ptr) -> lowercase_tim_string_ptr
		// Converts a Tim string to lowercase
		// Argument: rdi = Tim string pointer (as integer)
		// Returns: xmm0 = lowercase Tim string pointer (as float64)
		...
		// Generate trim_string(tim_string_ptr) -> trimmed_tim_string_ptr
		// Removes leading and trailing whitespace
		// Argument: rdi = Tim string pointer (as integer)
		// Returns: xmm0 = trimmed Tim string pointer (as float64)
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_string_eq used

	// Generate arena functions only if arenas are actually used
	if fc.usesArenas {
		if VerboseMode {
			debugf("DEBUG: Generating arena functions (usesArenas=true)\n")
		}
		// Define error message string
		fc.eb.Define("_tim_str_arena_alloc_error", "ERROR: Arena buffer allocation failed\n\x00")

		// Generate _tim_arena_create(capacity) -> arena_ptr
		...
		fc.eb.MarkLabel("_tim_arena_create")
		...
		if fc.eb.target.OS() == OSWindows {
			shadowSpace := fc.allocateShadowSpace()
			fc.out.LeaSymbolToReg("rcx", "_tim_str_arena_alloc_error")
			fc.callFunction("printf", "")
			fc.deallocateShadowSpace(shadowSpace)
			shadowSpace = fc.allocateShadowSpace()
			fc.out.MovImmToReg("rcx", "1")
			...
			fc.out.LeaSymbolToReg("rdi", "_tim_str_arena_alloc_error")
			fc.callFunction("printf", "")
			fc.out.MovImmToReg("rdi", "1")
			fc.callFunction("exit", "")
		}

		// Generate _tim_arena_alloc(arena_ptr, size) -> allocation_ptr
		// Allocates memory from the arena using bump allocation with auto-growing
		// If arena is full, reallocs buffer to 2x size
		// Arguments (internal ABI, all platforms):
		//   Windows: rcx = arena_ptr, rdx = size  (set by callArenaAlloc)
		//   Linux:   rdi = arena_ptr, rsi = size  (set by callArenaAlloc)
		// Returns: rax = allocated memory pointer
		fc.eb.MarkLabel("_tim_arena_alloc")
		...
		// Read arguments from the platform-correct registers (set by callArenaAlloc).
		fc.out.MovRegToReg("rbx", fc.argReg(0)) // rbx = arena_ptr
		fc.out.MovRegToReg("r12", fc.argReg(1)) // r12 = size
		...
		// Generate _tim_arena_destroy(arena_ptr)
		// Frees all memory associated with the arena
		// Argument: rdi = arena_ptr
		fc.eb.MarkLabel("_tim_arena_destroy")
	...
	if fc.usedFunctions["_tim_list_cons"] {
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_cons used

	// Generate _tim_list_head only if used
	if fc.usedFunctions["_tim_list_head"] {
		// Generate _tim_list_head(list_ptr_float) -> element_float
		...
		fc.eb.MarkLabel("_tim_list_head")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_head used

	// Generate _tim_list_tail only if used
	if fc.usedFunctions["_tim_list_tail"] {
		// Generate _tim_list_tail(list_ptr_float) -> tail_ptr_float
		// LINKED LIST implementation - returns tail of cons cell (O(1) operation)
		// Argument: rdi = list pointer (as float64 bits, 0.0 = nil)
		// Returns: xmm0 = tail pointer (as float64, 0.0 = nil)
		fc.eb.MarkLabel("_tim_list_tail")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_tail used

	// Generate _tim_list_length only if used
	if fc.usedFunctions["_tim_list_length"] {
		// Generate _tim_list_length(list_ptr_float) -> length_int
		// LINKED LIST implementation - walks list and counts nodes (O(n))
		// Argument: rdi = list pointer (as float64 bits, 0.0 = nil)
		// Returns: rax = length as int64
		fc.eb.MarkLabel("_tim_list_length")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_length used

	// Generate _tim_list_index only if used
	if fc.usedFunctions["_tim_list_index"] {
		// Generate _tim_list_index(list_ptr_float, index_int) -> element_float
		...
		fc.eb.MarkLabel("_tim_list_index")
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_index used

	// Generate _tim_list_update only if used
	if fc.usedFunctions["_tim_list_update"] {
		// Generate _tim_list_update(list_ptr, index, value, arena_ptr) -> new_list_ptr
		...
		fc.eb.MarkLabel("_tim_list_update")
		...
		// Call _tim_arena_alloc(rdi=arena_ptr, rsi=size)
		...
		fc.out.PopReg("rbp")
		fc.out.Ret()
	} // end if _tim_list_update used

	// Generate _tim_string_println only if used
	if fc.usedFunctions["_tim_string_println"] {
		// Generate _tim_string_println(string_ptr) - prints string followed by newline
		// Argument: rdi/rcx (platform-dependent) = string pointer (map with [count][0][char0][1][char1]...)
		fc.eb.MarkLabel("_tim_string_println")
		...
		if fc.eb.target.OS() == OSWindows {
			...
			charFmtLabel := fmt.Sprintf("_tim_char_fmt_%d", fc.stringCounter)
			fc.stringCounter++
			...
			newlineFmtLabel := fmt.Sprintf("_tim_newline_fmt_%d", fc.stringCounter)
			fc.stringCounter++
	...
	} // end if _tim_string_println used

	// Generate _tim_string_print only if used
	if fc.usedFunctions["_tim_string_print"] {
		// Generate _tim_string_print(string_ptr) - prints string WITHOUT newline
		// Argument: rdi/rcx (platform-dependent) = string pointer (map with [count][0][char0][1][char1]...)
		fc.eb.MarkLabel("_tim_string_print")
		...
		if fc.eb.target.OS() == OSWindows {
			...
			// Call putchar via printf
			charFmtLabel2 := fmt.Sprintf("_tim_char_fmt_%d", fc.stringCounter)
			fc.stringCounter++
	...
	} // end if _tim_string_print used

	// Generate _tim_itoa for number to string conversion (only if used)
	if fc.usedFunctions["_tim_itoa"] {
		fc.generateItoa()
	}
	...
	// Generate _tim_arena_ensure_capacity if arenas are used
	if fc.usesArenas {
...
func (fc *TimCompiler) initializeMetaArenaAndGlobalArena() {
	...
	fc.out.LeaSymbolToReg("rbx", "_tim_arena_meta_cap")
	...
	fc.out.MovImmToReg("rcx", "1")
	fc.out.LeaSymbolToReg("rbx", "_tim_arena_meta_len")
	fc.out.MovRegToMem("rcx", "rbx", 0)
}

// generateArenaInitFunction generates a standalone function _tim_init_arenas
// that initializes the meta-arena system. This should be called from _start before main.
func (fc *TimCompiler) generateArenaInitFunction() {
	fc.eb.MarkLabel("_tim_init_arenas")
...
func (fc *TimCompiler) cleanupAllArenas() {
	...
	// fc.out.LeaSymbolToReg("rax", "_tim_arena_meta_len")
...
// generateArenaEnsureCapacity generates the _tim_arena_ensure_capacity function
// This function ensures the meta-arena has enough capacity for the requested depth
// Argument: rdi = required_depth
func (fc *TimCompiler) generateArenaEnsureCapacity() {
	fc.eb.MarkLabel("_tim_arena_ensure_capacity")
...
func (fc *TimCompiler) compileStoredFunctionCall(call *CallExpr) {
	...
	if isVariadic {
		variadicArgCount := max(len(call.Args)-fixedParamCount, 0)
...
func (fc *TimCompiler) compileLambdaDirectCall(call *CallExpr) {
...
func (fc *TimCompiler) compileMemoizedCall(call *CallExpr, lambda *LambdaFunc) {
	...
	// rep movsq: rdi=dest, rsi=src, rcx=qwords. r13/r14/rbx/r12/r14 are callee-saved and unaffected.
	fc.out.MovRegToReg("rdi", "r13")
	fc.out.MovRegToReg("rsi", "rbx")
	fc.out.MovRegToReg("rcx", "rdx")
	fc.out.ShrImmReg("rcx", 3) // bytes → qwords
	fc.out.Cld()
	fc.out.RepMovsq()
...
func (fc *TimCompiler) compileBinaryOpSafe(left, right Expression, operator string) {
...
func (fc *TimCompiler) compileDirectCall(call *DirectCallExpr) {
	if VerboseMode {
		debugf("DEBUG compileDirectCall: callee type = %T\n", call.Callee)
		if v, ok := call.Callee.(*IdentExpr); ok {
			debugf("DEBUG compileDirectCall: callee ident name = '%s'\n", v.Name)
		}
	}
	...
	if unary, ok := call.Callee.(*UnaryExpr); ok {
		if VerboseMode {
			debugf("DEBUG compileDirectCall: detected operator '%s' via UnaryExpr, args=%d\n", unary.Operator, len(call.Args))
			for i, arg := range call.Args {
				debugf("DEBUG compileDirectCall: arg[%d] = %T\n", i, arg)
			}
			debugf("DEBUG compileDirectCall: UnaryExpr.Operand = %T\n", unary.Operand)
		}
	...
	if call.Callee == nil {
		if VerboseMode {
			debugf("DEBUG compileDirectCall: nil Callee with %d args\n", len(call.Args))
			for i, arg := range call.Args {
				debugf("DEBUG compileDirectCall: arg[%d] type = %T\n", i, arg)
			}
		}
		...
		if len(call.Args) == 1 {
			if innerCall, ok := call.Args[0].(*DirectCallExpr); ok {
				if VerboseMode {
					debugf("DEBUG compileDirectCall: compiling nested DirectCallExpr\n")
				}
	...
	// Special case: calling a value (not a function) just returns the value
	if len(call.Args) == 0 {
		isCallable := false
		...
		case *IdentExpr:
			if ident, ok := call.Callee.(*IdentExpr); ok {
				if fc.functionSignatures[ident.Name] != nil {
					isCallable = true
					if VerboseMode {
						debugf("DEBUG: DirectCall with 0 args - ident '%s' is callable\n", ident.Name)
					}
				} else {
					if VerboseMode {
						debugf("DEBUG: DirectCall with 0 args - ident '%s' is a plain value\n", ident.Name)
					}
				}
			}
		}
		...
		if !isCallable {
			if VerboseMode {
				debugf("DEBUG: Calling a plain value - just returning the value\n")
			}
			fc.compileExpression(call.Callee)
			return
		}
		if VerboseMode {
			debugf("DEBUG: Calling a lambda with 0 args - will dereference and call\n")
		}
	}
...
func (fc *TimCompiler) compileFloatToString(xmmReg, bufPtr string) {
...
func (fc *TimCompiler) loadFloatConstant(xmmReg string, value float64) {
...
func (fc *TimCompiler) compileIntToStringAtPos(intReg, posReg string) {
	fc.compileWholeNumberToStringAtPos(intReg, posReg, true)
}

// compileIntToStringAtPosNoNewline writes integer without newline
func (fc *TimCompiler) compileIntToStringAtPosNoNewline(intReg, posReg string) {
...
func (fc *TimCompiler) compileWholeNumberToStringAtPos(intReg, posReg string, addNewline bool) {
...
// Output: rsi = string start, rdx = length (including newline)
func (fc *TimCompiler) compileWholeNumberToString(intReg, bufPtr string) {
	// Special case: zero
...
func (fc *TimCompiler) compileRecursiveCall(call *CallExpr) {
...
func (fc *TimCompiler) compileCFunctionCall(libName string, funcName string, args []Expression) {
	// Generate C FFI call
	// Strategy for v1.1.0:
	...
	// Normalize empty namespace (c.foo) to libc for dependency/link tracking.
	trackedLib := libName
	if trackedLib == "" {
		trackedLib = "c"
	}
	...
	fc.cLibHandles[trackedLib] = "linked" // Mark as needing dynamic linking

	// Track function usage for PLT generation and call order patching
	fc.trackFunctionCall(funcName)

	// Track which library this function belongs to (for linker selection and imports)
	fc.cFunctionLibs[funcName] = trackedLib
	...
	// Library-specific fallback signatures when header parsing is incomplete.
	if funcSig == nil {
		switch strings.ToLower(libName) {
		case "gl", "opengl", "opengl32":
			glFunctions := map[string]*CFunctionSignature{
				"glViewport":      {ReturnType: "void", Params: []CFunctionParam{{Type: "int"}, {Type: "int"}, {Type: "int"}, {Type: "int"}}},
				"glEnable":        {ReturnType: "void", Params: []CFunctionParam{{Type: "uint32"}}},
				"glClear":         {ReturnType: "void", Params: []CFunctionParam{{Type: "uint32"}}},
				"glClearColor":    {ReturnType: "void", Params: []CFunctionParam{{Type: "GLclampf"}, {Type: "GLclampf"}, {Type: "GLclampf"}, {Type: "GLclampf"}}},
				"glMatrixMode":    {ReturnType: "void", Params: []CFunctionParam{{Type: "uint32"}}},
				"glLoadIdentity":  {ReturnType: "void", Params: []CFunctionParam{}},
				"glFrustum":       {ReturnType: "void", Params: []CFunctionParam{{Type: "GLdouble"}, {Type: "GLdouble"}, {Type: "GLdouble"}, {Type: "GLdouble"}, {Type: "GLdouble"}, {Type: "GLdouble"}}},
				"glShadeModel":    {ReturnType: "void", Params: []CFunctionParam{{Type: "uint32"}}},
				"glColorMaterial": {ReturnType: "void", Params: []CFunctionParam{{Type: "uint32"}, {Type: "uint32"}}},
				"glTranslatef":    {ReturnType: "void", Params: []CFunctionParam{{Type: "GLfloat"}, {Type: "GLfloat"}, {Type: "GLfloat"}}},
				"glRotatef":       {ReturnType: "void", Params: []CFunctionParam{{Type: "GLfloat"}, {Type: "GLfloat"}, {Type: "GLfloat"}, {Type: "GLfloat"}}},
				"glColor3f":       {ReturnType: "void", Params: []CFunctionParam{{Type: "GLfloat"}, {Type: "GLfloat"}, {Type: "GLfloat"}}},
				"glBegin":         {ReturnType: "void", Params: []CFunctionParam{{Type: "uint32"}}},
				"glNormal3f":      {ReturnType: "void", Params: []CFunctionParam{{Type: "GLfloat"}, {Type: "GLfloat"}, {Type: "GLfloat"}}},
				"glVertex3f":      {ReturnType: "void", Params: []CFunctionParam{{Type: "GLfloat"}, {Type: "GLfloat"}, {Type: "GLfloat"}}},
				"glEnd":           {ReturnType: "void", Params: []CFunctionParam{}},
			}
			if sig, ok := glFunctions[funcName]; ok {
				funcSig = sig
				if VerboseMode {
					fmt.Fprintf(os.Stderr, "Using fallback OpenGL signature for %s\n", funcName)
				}
			}
		}
	}
	...
	if len(args) > 0 {
		...
		for i, arg := range args {
			...
			normalizedParamType := strings.TrimSpace(strings.TrimPrefix(paramType, "const "))
			isFloat32Param := normalizedParamType == "float" || normalizedParamType == "GLfloat" || normalizedParamType == "GLclampf"
			isFloat64Param := normalizedParamType == "double" || normalizedParamType == "GLdouble"

			// Decide whether this parameter should be treated as float or int
			if isFloat32Param || isFloat64Param {
				info.isFloatParam = true
			}
			info.isFloat32 = isFloat32Param || (paramType == "" && info.castType == "float")
		...
		for i := range args {
			...
			if isNullPointer && (castType == "ptr" || castType == "pointer" || castType == "cstr" || castType == "cstring") {
				...
				// Compile the inner expression (result in xmm0 for Tim values, rax for C strings)
			...
			if info.isFloatParam || castType == "float" || castType == "double" {
				if isNullPointer {
					...
					// C float params require float32 bit-patterns in XMM low 32 bits.
					if info.isFloat32 && fc.eb.target.Arch() == ArchX86_64 {
						// cvtsd2ss xmm0, xmm0
						fc.out.Write(0xF2)
						fc.out.Write(0x0F)
						fc.out.Write(0x5A)
				...
				case "cstr", "cstring":
					if isNullPointer {
						...
						// Runtime string (Tim map format) - need to convert to C string
						fc.out.SubImmFromReg("rsp", StackSlotSize)
						fc.out.MovXmmToMem("xmm0", "rsp", 0)
						fc.out.MovMemToReg("rax", "rsp", 0)
						fc.out.AddImmToReg("rsp", StackSlotSize)

						// Call _tim_string_to_cstr(map_ptr) -> char*
						fc.out.SubImmFromReg("rsp", StackSlotSize)
						fc.out.MovRegToMem("rax", "rsp", 0)
		...
		if returnType == "double" {
			// Result is already in xmm0 as double.
		} else if returnType == "float" {
			// C float return uses low 32 bits of xmm0; widen to Tim float64.
			if fc.eb.target.Arch() == ArchX86_64 {
				// cvtss2sd xmm0, xmm0
				fc.out.Write(0xF3)
				fc.out.Write(0x0F)
				fc.out.Write(0x5A)
			...
			// Pointer type - keep raw integer value but convert to float64 for Tim
			// (Tim internally represents everything as float64)
			...
			// Integer result in rax - convert to float64 for Tim
...
func (fc *TimCompiler) compileCall(call *CallExpr) {
	if VerboseMode {
	...
	if strings.Contains(call.Function, ".") {
		...
		if len(parts) == 2 {
			...
			// Check if this is a Tim namespaced function call
			...
			if actualNamespace, exists := fc.functionNamespace[funcName]; exists && actualNamespace == namespace {
				// This is a valid namespaced Tim function call
	...
	if VerboseMode {
		debugf("DEBUG compileCall: function=%s, isBuiltinOp=%v\n", call.Function, isBuiltinOp)
	}
	...
	if !isBuiltinOp {
		...
		if isForwardRef && !isVariable {
			...
			if VerboseMode {
				debugf("DEBUG compileCall: forward reference - direct call to label '%s'\n", call.Function)
			}
			fc.compileLambdaDirectCall(call)
			return
		}
		...
		if isVariable {
			if VerboseMode {
				debugf("DEBUG compileCall: taking compileStoredFunctionCall path\n")
			}
	...
	if VerboseMode {
		debugf("DEBUG compileCall: entering switch for function='%s'\n", call.Function)
	}
	switch call.Function {
	// Arithmetic operators (prefix notation: (- 8 6) means 8 - 6)
	case "+", "-", "*", "/", "mod", "%":
		if VerboseMode {
			debugf("DEBUG compileCall: matched arithmetic operator '%s'\n", call.Function)
		}
		if len(call.Args) != 2 {
	...
	case "_error_code_extract":
		...
		// Convert 4-byte code to Tim string
		...
		// Entry 2: key=2, value=char2 (bits 15:8 of codeInt)
	...
	case "print":
		...
		if strExpr, ok := arg.(*StringExpr); ok {
			...
			// String variable - call _tim_print_syscall helper
			fc.compileExpression(arg)
	...
	case "printf":
		...
		for i < len(runes) {
			if runes[i] == '%' && i+1 < len(runes) {
				...
				if next == '%' {
					...
					// %v = value format (uses %f for consistent 6-decimal-place output)
		...
		xmmArgCount := 0
		isWindows := fc.eb.target.OS() == OSWindows
		// floatSlots tracks the overall arg slot index (0-based) of each float arg.
		// On Windows x64, each variadic arg occupies a unified slot (rdx/xmm1=0,
		// r8/xmm2=1, r9/xmm3=2), so float args must be duplicated into the integer
		// register corresponding to their slot, not just the next sequential int reg.
		floatSlots := []int{}

		// Evaluate all arguments
		for i := 1; i < len(call.Args); i++ {
			argIdx := i - 1
			// On Windows use argIdx as the unified slot; on Linux use intArgCount.
			intRegIdx := intArgCount
			if isWindows {
				intRegIdx = argIdx
			}
			...
			if stringPositions[argIdx] {
				if strExpr, ok := call.Args[i].(*StringExpr); ok {
					...
					if intRegIdx < len(intRegs) {
						fc.out.LeaSymbolToReg(intRegs[intRegIdx], labelName)
					}
					if !isWindows {
						intArgCount++
					}
			...
			if integerPositions[argIdx] {
				if castExpr, ok := call.Args[i].(*CastExpr); ok && castExpr.Type == "number" {
					...
					if intRegIdx < len(intRegs) && intRegs[intRegIdx] != "rax" {
						fc.out.MovRegToReg(intRegs[intRegIdx], "rax")
					}
					if !isWindows {
						intArgCount++
					}
			...
			if boolPositions[argIdx] {
				...
				// 0.0 -> "no"
				if intRegIdx < len(intRegs) {
					fc.out.LeaSymbolToReg(intRegs[intRegIdx], noLabel)
				}
				...
				if intRegIdx < len(intRegs) {
					fc.out.LeaSymbolToReg(intRegs[intRegIdx], yesLabel)
				}
				...
				// %s: Tim string -> C string conversion
				// xmm0 contains pointer to Tim string map [count][key0][val0][key1][val1]...
				...
				fc.out.MovXmmToMem("xmm0", "rsp", 0)
				if isWindows {
					floatSlots = append(floatSlots, argIdx)
				}
			}
		}
		...
		extraFloatXmmRegs := []string{"xmm4", "xmm5", "xmm6", "xmm7", "xmm8", "xmm9", "xmm10", "xmm11"}
		extraXmmCount := 0
		for i := range numArgs {
			if !boolPositions[i] && !stringPositions[i] && !integerPositions[i] {
				...
				} else if isWindows && extraXmmCount < len(extraFloatXmmRegs) {
					// Save extra arg to scratch xmm reg; will be placed on call stack later
					fc.out.MovMemToXmm(extraFloatXmmRegs[extraXmmCount], "rsp", offset)
					extraXmmCount++
				}
				xmmArgCount++
			}
		...
		// Each float arg occupies a unified slot with its corresponding integer register:
		// slot 0→rdx/xmm1, slot 1→r8/xmm2, slot 2→r9/xmm3.
		// Use floatSlots to map each float's xmm register to the correct integer register for its slot.
		if isWindows {
			for floatIdx, slot := range floatSlots {
				if floatIdx < len(xmmRegs) && slot < len(intRegs) {
					fc.out.MovqXmmToReg(intRegs[slot], xmmRegs[floatIdx])
				}
			}
		}
		...
		// Windows x64 calling convention requires 32 bytes of shadow space before any call.
		// For variadic printf on Windows, extra float args (beyond 3) go at [rsp+32+], [rsp+40+], etc.
		if fc.eb.target.OS() == OSWindows {
			allocSize := int64(32 + extraXmmCount*8)
			fc.out.SubImmFromReg("rsp", allocSize)
			for ei := 0; ei < extraXmmCount; ei++ {
				fc.out.MovXmmToMem(extraFloatXmmRegs[ei], "rsp", 32+ei*8)
			}
		}
		fc.callFunction("printf", "")
		if fc.eb.target.OS() == OSWindows {
			fc.out.AddImmToReg("rsp", int64(32+extraXmmCount*8))
		}
	...
	case "eprint", "eprintln", "eprintf":
		...
		if isFormatted {
			...
			if len(call.Args) == 0 {
				...
				fc.stringCounter++
				fc.eb.Define(newlineLabel, "\n")
				fc.emitStderrWriteLiteral(newlineLabel, 1, isWindows)
			} else {
				arg := call.Args[0]
				if strExpr, ok := arg.(*StringExpr); ok {
					labelName := fmt.Sprintf("str_%d", fc.stringCounter)
					...
					fc.out.SubImmFromReg("rsp", 32)
					fc.callFunction("_tim_itoa", "")
					fc.emitStderrWriteRegBuf(isWindows) // rsi=buf, rdx=len
					newlineLabel := fmt.Sprintf("eprintln_newline_%d", fc.stringCounter)
					fc.stringCounter++
					fc.eb.Define(newlineLabel, "\n")
					fc.emitStderrWriteLiteral(newlineLabel, 1, isWindows)
			...
			if strExpr, ok := arg.(*StringExpr); ok {
				...
				fc.out.SubImmFromReg("rsp", 32)
				fc.callFunction("_tim_itoa", "")
				fc.emitStderrWriteRegBuf(isWindows)
				fc.out.AddImmToReg("rsp", 32)
			}
		}
	...
	case "exitln", "exitf":
		...
		if isFormatted {
			...
			if fc.platform.OS == OSWindows {
				if len(call.Args) == 1 {
					// Simple case: just a format string with no additional args - write to stderr
					fc.emitStderrWriteLiteral(labelName, len(fmtStr), true)
				} else {
					// With args: use printf to stdout (TODO: proper stderr support with format args)
					shadowSpace := fc.allocateShadowSpace()
					...
					for argIdx := 1; argIdx < len(call.Args) && argIdx <= 8; argIdx++ {
						...
						if boolPositions[argIdx-1] {
							...
							fc.compileExpression(arg)
							if needsConversion {
								fc.callFunction("_tim_string_to_cstr", "")
				...
				if len(call.Args) == 1 {
					...
					for argIdx := 1; argIdx < len(call.Args) && argIdx <= 8; argIdx++ {
						...
						if stringPositions[argIdx-1] {
							...
							if needsConversion {
								// Convert Tim string to C string
								fc.callFunction("_tim_string_to_cstr", "")
							} else {
			...
			isWin := fc.eb.target.OS() == OSWindows
			if len(call.Args) == 0 {
				newlineLabel := fmt.Sprintf("exitln_newline_%d", fc.stringCounter)
				fc.stringCounter++
				fc.eb.Define(newlineLabel, "\n")
				fc.emitStderrWriteLiteral(newlineLabel, 1, isWin)
			} else {
				arg := call.Args[0]
				if strExpr, ok := arg.(*StringExpr); ok {
					labelName := fmt.Sprintf("str_%d", fc.stringCounter)
					...
					fc.out.SubImmFromReg("rsp", 32)
					fc.callFunction("_tim_itoa", "")
					fc.emitStderrWriteRegBuf(isWin)
					newlineLabel := fmt.Sprintf("exitln_newline_%d", fc.stringCounter)
					fc.stringCounter++
					fc.eb.Define(newlineLabel, "\n")
					fc.emitStderrWriteLiteral(newlineLabel, 1, isWin)
	...
	case "append":
		...
		// rep movsq: rdi=dest, rsi=src, rcx=qwords (clobbers rdi,rsi,rcx; preserves rax)
		fc.out.PushReg("rcx")               // save old count (rep movsq clobbers rcx)
		fc.out.LeaMemToReg("rdi", "rax", 8) // dest = new_list + 8
		fc.out.LeaMemToReg("rsi", "rsi", 8) // src = old_list + 8
		fc.out.ShlImmReg("rcx", 1)          // rcx = old_count * 2 (each entry = 16 bytes = 2 qwords)
		fc.out.Cld()
		fc.out.RepMovsq()

		// Restore old count; rax still = new_list (not clobbered by rep movsq)
		fc.out.PopReg("rcx")
	...
	case "pop":
		...
		// rep movsq: rbx/r12/r13/r14/r15 are callee-saved, not clobbered by movsq.
		fc.out.LeaMemToReg("rdi", "r14", 8) // dest = new_list + 8
		fc.out.LeaMemToReg("rsi", "rbx", 8) // src = input_list + 8
		fc.out.MovRegToReg("rcx", "r13")
		fc.out.ShlImmReg("rcx", 1) // rcx = (count-1) * 2 qwords
		fc.out.Cld()
		fc.out.RepMovsq()
	...
	case "arena_create":
		...
		fc.out.Cvttsd2si("rdi", "xmm0")
		fc.out.CallSymbol("_tim_arena_create")
	...
	case "arena_destroy":
		...
		fc.compileExpression(call.Args[0])
		fc.out.Cvttsd2si("rdi", "xmm0")
		fc.out.CallSymbol("_tim_arena_destroy")
	...
	case "log10":
		if len(call.Args) != 1 {
			compilerError("log10() requires exactly 1 argument")
		}
		fc.compileExpression(call.Args[0])
		// log10(x) = log10(2) * log2(x)
		// FYL2X computes ST(1) * log2(ST(0)), so with ST(1)=log10(2), ST(0)=x
		// the result is log10(2) * log2(x) = log10(x)
		fc.out.SubImmFromReg("rsp", StackSlotSize)
		fc.out.MovXmmToMem("xmm0", "rsp", 0)
		fc.out.Fldlg2()         // ST(0) = log10(2)
		fc.out.FldMem("rsp", 0) // ST(0) = x, ST(1) = log10(2)
		fc.out.Fyl2x()          // ST(0) = log10(2) * log2(x) = log10(x)
		fc.out.FstpMem("rsp", 0)
		fc.out.MovMemToXmm("xmm0", "rsp", 0)
		fc.out.AddImmToReg("rsp", StackSlotSize)
	...
	case "min":
		if len(call.Args) != 2 {
			compilerError("min() requires exactly 2 arguments")
		}
		fc.compileExpression(call.Args[0]) // a in xmm0
		fc.out.SubImmFromReg("rsp", StackSlotSize)
		fc.out.MovXmmToMem("xmm0", "rsp", 0)
		...
		fc.out.MovMemToXmm("xmm0", "rsp", 0)
		fc.out.AddImmToReg("rsp", StackSlotSize)
		fc.out.MinsdXmm("xmm0", "xmm1") // xmm0 = min(a, b)

	case "max":
		if len(call.Args) != 2 {
			compilerError("max() requires exactly 2 arguments")
		}
		fc.compileExpression(call.Args[0]) // a in xmm0
		fc.out.SubImmFromReg("rsp", StackSlotSize)
		fc.out.MovXmmToMem("xmm0", "rsp", 0)
		...
		fc.out.MovMemToXmm("xmm0", "rsp", 0)
		fc.out.AddImmToReg("rsp", StackSlotSize)
		fc.out.MaxsdXmm("xmm0", "xmm1") // xmm0 = max(a, b)

	// ===== RAYLIB GRAPHICS BUILTINS =====
	// These call directly into the system raylib shared library (libraylib)
	// via the C FFI mechanism, so no external Tim wrapper package is required.
	case "InitWindow", "CloseWindow", "DrawRectangle",
		"BeginDrawing", "EndDrawing", "ClearBackground",
		"WindowShouldClose", "SetTargetFPS", "DrawPixel",
		"DrawText", "DrawCircle", "DrawLine":
		fc.compileCFunctionCall("raylib", call.Function, call.Args)
	...
	case "str":
		...
		// str(x) converts a number to a Tim string (map[uint64]float64)
		if len(call.Args) != 1 {
	...
	case "num":
		...
		// num(string) converts a Tim string to a number
		if len(call.Args) != 1 {
			compilerError("num() requires exactly 1 argument")
		}

		// Compile argument (Tim string pointer in xmm0)
		fc.compileExpression(call.Args[0])
		...
		// rdi = C string (already in rax from _tim_string_to_cstr)
		fc.out.MovRegToReg("rdi", "rax")
	...
	case "peek8":
		// peek8(ptr, offset) - Read uint8 from pointer + offset
		// ptr: pointer (as float64), offset: byte offset (as float64)
		// Returns: uint8 value (0..255) as float64
		if len(call.Args) != 2 {
			compilerError("peek8() requires 2 arguments (ptr, offset)")
		}

		// Read 4 bytes via the same mechanism as peek32, then mask to 8 bits.
		// This keeps the codegen tiny and matches the peek32 calling pattern.
		fc.compileExpression(call.Args[0])
		fc.out.Cvttsd2si("rdi", "xmm0") // ptr in rdi
		fc.compileExpression(call.Args[1])
		fc.out.Cvttsd2si("rsi", "xmm0") // offset in rsi
		fc.out.AddRegToReg("rdi", "rsi")
		fc.out.MovMemToReg("eax", "rdi", 0) // load 32 bits
		fc.out.AndRegWithImm("rax", 0xFF)   // mask low byte
		fc.out.Cvtsi2sd("xmm0", "rax")
	...
	case "alloc":
		...
		// Load arena pointer from meta-arena: _tim_arena_meta[currentArena-1]
		...
		// Return pointers as numeric float64 values so regular pointer call sites
		// (which use cvttsd2si) round-trip correctly.
		fc.out.Cvtsi2sd("xmm0", "rax")
	...
	case "malloc":
		...
		// Compile size argument; callArenaAlloc() expects size in rdi and
		// handles the platform-specific register setup (rcx/rdx on Windows,
		// rdi/rsi on Linux) before calling _tim_arena_alloc.
		fc.compileExpression(call.Args[0])
		fc.out.Cvttsd2si("rdi", "xmm0") // size in rdi (callArenaAlloc convention)
		fc.callArenaAlloc()
		// Return pointers as numeric float64 values so regular pointer call sites
		// (which use cvttsd2si) round-trip correctly.
		fc.out.Cvtsi2sd("xmm0", "rax")
	...
	case "dlopen":
		...
		// path: string (Tim string), flags: number (RTLD_LAZY=1, RTLD_NOW=2)
		...
		// Evaluate path argument (Tim string)
		fc.compileExpression(call.Args[0])
		// Convert Tim string to C string (xmm0 has map pointer)
		...
		// Call _tim_string_to_cstr (result in rax)
		fc.trackFunctionCall("_tim_string_to_cstr")
		fc.out.CallSymbol("_tim_string_to_cstr")
	...
	case "readln":
		...
		// Convert C string to Tim string
		...
		// Create empty Tim string (count = 0)
	...
	case "read_file":
		// read_file(path) - Read entire file, return as Tim string
		...
		// Convert Tim string to C string (null-terminated)
		fc.out.SubImmFromReg("rsp", StackSlotSize)
		fc.out.MovXmmToMem("xmm0", "rsp", 0)
		fc.out.MovMemToReg("rdi", "rsp", 0)
		fc.out.AddImmToReg("rsp", StackSlotSize)
		...
		fc.out.MovRegToReg("rdi", "rax")   // path from _tim_string_to_cstr
	...
	case "__tim_map_update":
		// __tim_map_update(list, index, value) - Update a list element (functional update)
		// Calls runtime function: tim_list_update(list_ptr, index, value)
		// Arguments: rdi=list_ptr, rsi=index, xmm0=value
		// Returns: rax=new_list_ptr (converted to xmm0)
		if len(call.Args) != 3 {
			compilerError("__tim_map_update() requires exactly 3 arguments (list, index, value)")
		}
		...
		// Now setup arguments for tim_list_update(rdi=list_ptr, rsi=index, xmm0=value, rcx=arena_ptr)
		// Pop index into rsi
		...
		fc.out.LeaSymbolToReg("rdx", "_tim_arena_meta")
		...
		fc.callFunction("_tim_list_update", "")
	...
	default:
		if VerboseMode {
			debugf("DEBUG: Hit default case for function='%s'\n", call.Function)
		}
...
func (fc *TimCompiler) createErrorResult(errorCode string) {
...
func (fc *TimCompiler) trackFunctionCall(funcName string) {
...
func (fc *TimCompiler) callFunction(funcName string, library string) error {
	fc.trackFunctionCall(funcName)
	if library != "" {
...
func collectFunctionCallsFromStmtWithParams(stmt Statement, calls map[string]bool, params map[string]bool) {
	...
	case *IfStmt:
		for _, branch := range s.Branches {
			collectFunctionCallsWithParams(branch.Condition, calls, params)
			for _, bodyStmt := range branch.Body {
				collectFunctionCallsFromStmtWithParams(bodyStmt, calls, params)
			}
		}
...
func collectDefinedFromStmt(stmt Statement, defined map[string]bool) {
	if assign, ok := stmt.(*AssignStmt); ok {
		...
		for _, branch := range ifStmt.Branches {
			collectDefinedFromExpr(branch.Condition, defined)
			for _, bodyStmt := range branch.Body {
				collectDefinedFromStmt(bodyStmt, defined)
			}
		}
		for _, bodyStmt := range ifStmt.ElseBody {
			collectDefinedFromStmt(bodyStmt, defined)
		}
	}
}
...
func getUnknownFunctions(program *Program) []string {
	...
	// Collect Tim import namespaces (e.g., "lib", "math")
	timImports := make(map[string]bool)
	for _, stmt := range program.Statements {
		if imp, ok := stmt.(*ImportStmt); ok {
			if imp.Alias != "*" {
				timImports[imp.Alias] = true
			}
		}
	}
	...
	for funcName := range calls {
		...
		// Check for Tim import namespaces
		isFromTimImport := false
		for ns := range timImports {
			if len(funcName) > len(ns)+1 && funcName[:len(ns)+1] == ns+"." {
				isFromTimImport = true
				break
			}
		}
		...
		if !builtins[funcName] && !defined[funcName] && !isFromCImport && !isFromTimImport && !isMethodCall {
...
func processImports(program *Program, platform Platform, sourceFilePath string) error {
	...
	if len(imports) == 0 {
		return nil // No Tim imports to process
	}
	...
	for _, imp := range imports {
		...
		timFiles, err := ResolveImport(spec, platform.OS.String(), platform.Arch.String())
		if err != nil {
		...
		for _, f := range timFiles {
			absF, _ := filepath.Abs(f)
			if absF != absSourcePath {
				filteredFiles = append(filteredFiles, f)
			}
		}
		timFiles = filteredFiles

		// Parse and merge each .tim file with namespace handling
		for _, timFile := range timFiles {
			depContent, err := os.ReadFile(timFile)
			if err != nil {
				if VerboseMode {
			...
			depParser := NewParserWithFilename(string(depContent), timFile)
			...
			if depProgram.FunctionNamespaces != nil {
				...
				maps.Copy(program.FunctionNamespaces, depProgram.FunctionNamespaces)
			}
	...
	if envBool("DEBUG") {
		if VerboseMode {
			debugf("DEBUG processImports: final program after import processing:\n")
		}
		for i, stmt := range program.Statements {
			if VerboseMode {
...
func CompileTim(inputPath string, outputPath string, platform Platform) (err error) {
	return CompileTimWithOptions(inputPath, outputPath, platform, WPOTimeout, VerboseMode, false)
}

func CompileTimWithOptions(inputPath string, outputPath string, platform Platform, wpoTimeout float64, verbose bool, depsOnly bool) (err error) {
	// Set verbose mode
	...
	defer func() {
		if r := recover(); r != nil {
			...
			if VerboseMode {
				debugf("DEBUG: Panic stack trace:\n")
				debug.PrintStack()
			}
			if e, ok := r.(error); ok {
				err = e
	...
	if VerboseMode {
		...
		debugf("DEBUG: Parsed program (String() disabled to avoid crash)\n")
	}
	...
	if len(unknownFuncs) > 0 {
		...
		// First, try to load sibling .tim files from the same directory
		...
		// Only skip /tmp if there are many .tim files (likely temp files from -c flag)
		...
		if (inputDir == "/tmp" || inputDir == "C:\\tmp" || strings.HasPrefix(inputDir, "/tmp/") || strings.HasPrefix(inputDir, "C:\\tmp\\")) && err == nil {
			// Count .tim files - if there are more than 10, likely temp files
			timCount := 0
			for _, entry := range dirEntries {
				if !entry.IsDir() && strings.HasSuffix(entry.Name(), ".tim") {
					timCount++
					if timCount > 10 {
						skipSiblings = true
						break
		...
		if err == nil && !skipSiblings {
			...
			for _, entry := range dirEntries {
				name := entry.Name()
				// Include .tim files in same directory (except the input file itself)
				if !entry.IsDir() && strings.HasSuffix(name, ".tim") && name != inputBase {
		...
		if len(repos) > 0 {
			...
			for _, repoURL := range repos {
				...
				// Find all .tim files in the repository
				timFiles, err := FindTimFiles(repoPath)
				if err != nil {
					return fmt.Errorf("failed to find .tim files in %s: %v", repoPath, err)
				}

				// Parse and merge each .tim file
				for _, timFile := range timFiles {
					depContent, err := os.ReadFile(timFile)
					if err != nil {
						if VerboseMode {
	...
	// Compile
	compiler, err := NewTimCompiler(platform, verbose)
	if err != nil {
		return fmt.Errorf("failed to create compiler: %v", err)
	}
	compiler.sourceCode = combinedSource
	compiler.sourceDir = filepath.Dir(inputPath)
...
func (fc *TimCompiler) PrintDependencyInfo() {
...
func (fc *TimCompiler) compileARM64(program *Program, outputPath string) error {
