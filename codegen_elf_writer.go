...
func (fc *TimCompiler) writeELF(program *Program, outputPath string) error {
	// Check if dynamic linking is actually needed
	// On Linux, printf/println use syscalls, not libc
	libcFunctions := map[string]bool{
		// Note: printf NOT in list - Tim implements it with syscalls on Linux
	...
	if needsStatic {
		...
		debugf("DEBUG: Collecting writable constants from fc.eb.consts (%d total)\n", len(fc.eb.consts))
		for name, c := range fc.eb.consts {
			debugf("DEBUG:   %s: writable=%v, value_len=%d\n", name, c.writable, len(c.value))
			if c.writable {
		...
		debugf("DEBUG: Found %d writable constants\n", len(writableConsts))
		...
		debugf("DEBUG: Assigning data symbol addresses: dataAddr=0x%x, size=%d, numSymbols=%d\n",
			dataAddr, dataSize, len(dataSymbols))
		debugf("DEBUG: dataSymbols: %v\n", dataSymbols)
		for name, offset := range dataSymbols {
			addr := dataAddr + uint64(offset)
			fc.eb.DefineAddr(name, addr)
			debugf("DEBUG:   %s -> 0x%x (offset=%d)\n", name, addr, offset)
		}
		...
		if libName != "linked" {
			...
			if !strings.Contains(libSoName, ".so") {
				...
				if output, err := cmd.Output(); err == nil {
					...
					if after, ok := strings.CutPrefix(libs, "-l"); ok {
						libSoName = "lib" + after + ".so"
					} else {
	...
	// Initialize arena system only when arena-backed runtime paths are needed.
	// Unconditional emission introduces arena symbol relocations even for programs
	// that do not define arena metadata symbols.
	if fc.usesArenas {
	...
	if exists {
		// main exists - must be a function/lambda
	...
	if VerboseMode {
		debugf("DEBUG: Using syscall exit (no libc)\n")
	}
	...
	for symbol := range rodataSymbols {
		found := slices.Contains(symbolNames, symbol)
		if !found {
	...
	if VerboseMode {
		debugf("DEBUG: Setting lambda function addresses, have %d lambdas\n", len(fc.lambdaOffsets))
	}
	for lambdaName, offset := range fc.lambdaOffsets {
		...
		if fc.dynamicSymbols != nil {
			if VerboseMode {
				debugf("DEBUG: Calling UpdateSymbolValue for lambda '%s' at address 0x%x\n", lambdaName, lambdaAddr)
			}
			success := fc.dynamicSymbols.UpdateSymbolValue(lambdaName, lambdaAddr)
			if VerboseMode {
				debugf("DEBUG: UpdateSymbolValue returned %v\n", success)
			}
		} else if VerboseMode {
			debugf("DEBUG: fc.dynamicSymbols is nil, cannot update symbol\n")
		}
	}
