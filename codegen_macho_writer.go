...
func (fc *TimCompiler) resolveDylibPath(libName string) string {
...
func (fc *TimCompiler) writeMachOARM64(outputPath string) error {
	...
	for _, patch := range fc.eb.callPatches {
		...
		if strings.HasPrefix(funcName, "_tim_") || strings.HasPrefix(funcName, "tim_") || strings.HasPrefix(funcName, "__") {
			continue
		}
	...
	if VerboseMode {
		debugf("DEBUG: textNSects=%d, useDynamicLinking=%v, numImports=%d\n", textNSects, fc.eb.useDynamicLinking, numImports)
	}
	...
	if rodataSize > 0 || (fc.eb.useDynamicLinking && numImports > 0) {
		...
		if VerboseMode {
			debugf("DEBUG: dataNSects=%d, rodataSize=%d\n", dataNSects, rodataSize)
		}
	...
	if VerboseMode {
		debugf("DEBUG codegen_macho_writer: headerSize=%d, loadCmdsSize=%d, fileHeaderSize=%d, textSectAddr=0x%x\n",
	...
	if VerboseMode {
		debugf("DEBUG: textSegFileSize=0x%x, textSegVMSize=0x%x, rodataAddr=0x%x\n",
	...
	for labelName, offset := range fc.eb.labels {
		...
		if VerboseMode {
			debugf("DEBUG: Setting label %s address to 0x%x (offset %d)\n", labelName, labelAddr, offset)
		}
	}
	...
	for i, funcName := range fc.eb.neededFunctions {
		...
		if VerboseMode {
			debugf("DEBUG: Setting stub %s address to 0x%x\n", stubName, stubAddr)
		}
	}
