...
func (fc *TimCompiler) writePE(program *Program, outputPath string) error {
	if VerboseMode {
	...
	for funcName := range fc.usedFunctions {
		...
		// Skip Tim built-in functions (they generate internal code)
	...
	debugf("DEBUG: Windows imports by library:\n")
