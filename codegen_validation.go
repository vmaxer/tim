...
func (fc *TimCompiler) validateGeneratedCode() []string {
	var warnings []string
	...
	for funcName := range fc.usedFunctions {
		// Skip internal Tim runtime functions - they'll be generated
	...
	if fc.usesArenas {
		arenaFuncs := []string{"_tim_init_arenas", "_tim_arena_alloc", "_tim_arena_reset", "_tim_arena_ensure_capacity"}
	...
	if len(code) > 0 {
		...
		if offset >= 100 {
			...
			if VerboseMode {
				debugf("DEBUG: Found prologue at offset 0x%X\n", offset)
			}
		}
	}
...
func (fc *TimCompiler) printCodeValidation() {
	...
	if VerboseMode {
		if len(warnings) > 0 {
			...
			debugf("✓ Code validation passed - no warnings\n")
		}
	}
}
