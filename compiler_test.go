...
// TestBasicCompilation tests that Tim can compile and generate an executable
...
func TestExecutableGeneration(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("ELF executable permission test is Linux-only")
	}
	tmpfile, err := os.CreateTemp("", "tim_test_output")
	if err != nil {
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tmpFile, err := os.CreateTemp("", "tim_error_test_*.tim")
			if err != nil {
				t.Fatalf("Failed to create temp file: %v", err)
			}
			tmpFilePath := tmpFile.Name()
			...
			tmpOutput, err := os.CreateTemp("", "tim_error_output_*")
			if err != nil {
			...
			err = CompileTim(tmpFilePath, tmpOutputPath, platform)
