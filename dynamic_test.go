...
func TestDynamicELFStructure(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("Dynamic ELF structure test is Linux-only")
	}
	tmpfile, err := os.CreateTemp("", "tim_dynamic_test")
	if err != nil {
	...
	if err != nil {
		...
		foundLibc := slices.Contains(libs, "libc.so.6")
		if !foundLibc {
...
func TestLDDOutput(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("ldd test is Linux-only")
	}
	tmpfile, err := os.CreateTemp("", "tim_ldd_test")
	if err != nil {
		t.Fatalf("Failed to create temp file: %v", err)
	}
	tmpfilePath := tmpfile.Name()
