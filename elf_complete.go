...
func (eb *ExecutableBuilder) WriteCompleteDynamicELF(ds *DynamicSections, functions []string) (gotBase, rodataAddr, textAddr, pltBase uint64, err error) {
	...
	if VerboseMode {
		debugf("DEBUG [WriteCompleteDynamicELF start]: rodata buffer size: %d bytes\n", eb.rodata.Len())
	}
	if eb.rodata.Len() > 0 {
		previewLen := min(eb.rodata.Len(), 32)
		if VerboseMode {
			debugf("DEBUG [WriteCompleteDynamicELF start]: rodata buffer first %d bytes: %q\n", previewLen, eb.rodata.Bytes()[:previewLen])
		}
	}
	...
	if VerboseMode {
		debugf("DEBUG: codeSize=%d (0x%x), rodataSize=%d (0x%x)\n",
	...
	w.Write(3)    // Linux
	w.WriteN(0, 8)
	w.Write2(3) // DYN (PIE executable / shared object style)
	...
	if VerboseMode {
		debugf("DEBUG [after writing rodata to ELF]: rodata buffer size: %d bytes\n", eb.rodata.Len())
	}
	if eb.rodata.Len() > 0 {
		previewLen := min(eb.rodata.Len(), 32)
		if VerboseMode {
			debugf("DEBUG [after writing rodata to ELF]: rodata buffer first %d bytes: %q\n", previewLen, eb.rodata.Bytes()[:previewLen])
		}
	}
	...
	if VerboseMode {
		debugf("DEBUG [before writing data to ELF]: data offset in ELF = 0x%x\n", eb.dataOffsetInELF)
	}

	// Write .data section
	w.WriteBytes(eb.data.Bytes())
	if VerboseMode {
		debugf("DEBUG [after writing data to ELF]: data buffer size: %d bytes\n", eb.data.Len())
	}
	if eb.data.Len() > 0 {
		previewLen := min(eb.data.Len(), 32)
		if VerboseMode {
			debugf("DEBUG [after writing data to ELF]: data buffer first %d bytes: %x\n", previewLen, eb.data.Bytes()[:previewLen])
		}
	}
...
func (eb *ExecutableBuilder) patchPLTCalls(ds *DynamicSections, textAddr uint64, pltBase uint64, functions []string) {
	...
	if VerboseMode && len(textBytes) > 70 {
		debugf("DEBUG patchPLTCalls: Bytes at 63-68 in eb.text: %x\n", textBytes[63:68])
	}
}
...
func (eb *ExecutableBuilder) patchX86PLTCalls(textBytes []byte, ds *DynamicSections, textAddr, pltBase uint64, functions []string) {
	...
	if VerboseMode {
		debugf("DEBUG patchX86PLTCalls: have %d callPatches, textBytes len=%d\n", len(eb.callPatches), len(textBytes))
	}
	for _, patch := range eb.callPatches {
		if VerboseMode {
			debugf("DEBUG patchX86PLTCalls: patch at pos=%d, target=%s\n", patch.position, patch.targetName)
		}
		...
		if callPos < 0 {
			if VerboseMode {
				debugf("DEBUG: callPos %d < 0, skipping\n", callPos)
			}
			continue
		}
		if callPos >= len(textBytes) {
			if VerboseMode {
				debugf("DEBUG: callPos %d >= len(textBytes) %d, skipping\n", callPos, len(textBytes))
			}
			continue
		}
		if placeholderPos+3 >= len(textBytes) {
			if VerboseMode {
				debugf("DEBUG: placeholderPos+3 %d >= len(textBytes) %d, skipping\n", placeholderPos+3, len(textBytes))
			}
			continue
		}
		if textBytes[callPos] != 0xE8 {
			if VerboseMode {
				debugf("DEBUG: textBytes[%d] = 0x%x, not 0xE8, skipping\n", callPos, textBytes[callPos])
			}
			continue
		}
		...
		if true {
			...
			if VerboseMode {
				debugf("DEBUG: At pos %d, found 0xE8, checking placeholder: expected %x, got %x\n", callPos, placeholder, actualPlaceholder)
			}
			if bytes.Equal(textBytes[placeholderPos:placeholderPos+4], placeholder) {
				...
				if pltOffset >= 0 {
					...
					if VerboseMode {
						debugf("DEBUG: Patching PLT call to %s: plt=%d, targetAddr=%x\n", funcName, pltOffset, targetAddr)
					}
				} else if labelOffset, ok := eb.labels[funcName]; ok {
					// Internal label (e.g., tim_arena_alloc, _tim_arena_ensure_capacity)
					targetAddr = textAddr + uint64(labelOffset)
					isInternal = true
					if VerboseMode {
						debugf("DEBUG: Patching internal call to %s: labelOffset=%d, targetAddr=%x\n", funcName, labelOffset, targetAddr)
					}
				} else {
...
func (eb *ExecutableBuilder) patchARM64PLTCalls(textBytes []byte, ds *DynamicSections, textAddr, pltBase uint64, functions []string) {
	// Use callPatches which has both position and function name
	if VerboseMode {
		debugf("DEBUG patchARM64PLTCalls: have %d callPatches\n", len(eb.callPatches))
	}
	for _, patch := range eb.callPatches {
		if VerboseMode {
			debugf("DEBUG patchARM64PLTCalls: patch at pos=%d, target=%s\n", patch.position, patch.targetName)
		}
		// Extract function name from targetName (strip "$stub" suffix if present)
		funcName := strings.TrimSuffix(patch.targetName, "$stub")
		...
		if callPos < 0 || callPos+3 >= len(textBytes) {
			if VerboseMode {
				debugf("DEBUG: callPos %d out of bounds (len=%d), skipping\n", callPos, len(textBytes))
			}
			continue
		}
		...
		if (instr & 0xFC000000) != 0x94000000 {
			if VerboseMode {
				debugf("DEBUG: Not a BL instruction at %d: 0x%08x\n", callPos, instr)
			}
			continue
		}
