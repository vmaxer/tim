...
func (ds *DynamicSections) UpdateSymbolValue(name string, value uint64) bool {
	...
	if !exists {
		if VerboseMode {
			debugf("DEBUG UpdateSymbolValue: string '%s' not found in dynstrMap\n", name)
		}
		return false
	}

	if VerboseMode {
		debugf("DEBUG UpdateSymbolValue: looking for symbol '%s' with string offset %d, value=0x%x\n", name, targetName, value)
	}

	for i := range ds.dynsymSyms {
		if ds.dynsymSyms[i].name == targetName {
			if VerboseMode {
				debugf("DEBUG UpdateSymbolValue: found symbol at index %d, updating value from 0x%x to 0x%x, shndx from %d to 1\n",
	...
	if VerboseMode {
		debugf("DEBUG UpdateSymbolValue: symbol '%s' not found in symbol table\n", name)
	}
	return false
}
