...
func TestMinimalELFSize(t *testing.T) {
	tmpDir := t.TempDir()
	tmpfile := filepath.Join(tmpDir, "tim_size_test")
...
func TestDynamicELFExecutable(t *testing.T) {
	...
	tmpfile := filepath.Join(os.TempDir(), "tim_exec_test")
	defer os.Remove(tmpfile)
...
func TestELFSegmentAlignment(t *testing.T) {
	tmpDir := t.TempDir()
	tmpfile := filepath.Join(tmpDir, "tim_align_test")
	defer os.Remove(tmpfile)
...
func TestELFInterpSegment(t *testing.T) {
	tmpDir := t.TempDir()
	tmpfile := filepath.Join(tmpDir, "tim_interp_test")
	defer os.Remove(tmpfile)
...
func TestELFDynamicSegment(t *testing.T) {
	tmpDir := t.TempDir()
	tmpfile := filepath.Join(tmpDir, "tim_dynamic_seg_test")
	defer os.Remove(tmpfile)
...
func TestELFType(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("ELF ET_DYN test is Linux-only")
	}
	tmpDir := t.TempDir()
	tmpfile := filepath.Join(tmpDir, "tim_type_test")
	defer os.Remove(tmpfile)
...
func TestELFMachine(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.arch, func(t *testing.T) {
			tmpfile := filepath.Join(os.TempDir(), "tim_machine_test_"+tt.arch)
			defer os.Remove(tmpfile)
...
func TestELFPermissions(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("ELF permission test is Linux-only")
	}
	tmpDir := t.TempDir()
	tmpfile := filepath.Join(tmpDir, "tim_perms_test")
	defer os.Remove(tmpfile)
