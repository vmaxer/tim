...
func TestEmitCallInstruction(t *testing.T) {
	...
	// On Windows, external calls go through IAT (indirect CALL: 0xff 0x15 + 32-bit offset)
	// On Linux/macOS, direct relative CALL (0xe8 + 32-bit offset) is used
	if eb.target.OS() == OSWindows {
		if textBytes[0] != 0xff {
			t.Errorf("Expected indirect CALL opcode (0xff) on Windows, got 0x%x", textBytes[0])
		}
	} else {
		if textBytes[0] != 0xe8 {
			t.Errorf("Expected direct CALL opcode (0xe8), got 0x%x", textBytes[0])
		}
