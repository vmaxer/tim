...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tmpDir := t.TempDir()
			srcPath := filepath.Join(tmpDir, tt.name+".tim")
			...
			// Try to compile
			err := CompileTim(srcPath, outPath, platform)
			...
			// The key is that the Tim code compiles and generates valid assembly
			if err != nil {
				...
				if isLinkError(err) {
					...
					// This is OK - the Tim code compiled successfully
					return
				}
...
println("ENet test")
`
	srcPath := filepath.Join(tmpDir, "enet_server.tim")
	...
	err := CompileTim(srcPath, serverBin, platform)
