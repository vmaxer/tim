...
func (fc *TimCompiler) generateMetaArenaGrowth() {
...
func (fc *TimCompiler) generateArenaInitLoop() {
	...
	jumpOffset := int32(initLoopLabel - (fc.eb.text.Len() + UnconditionalJumpSize))
...
//	r15 = meta-arena pointer
func (fc *TimCompiler) generateFirstMetaArenaAlloc() {
	...
	fc.trackFunctionCall("malloc") // Will become _tim_arena_create
	fc.out.CallSymbol("_tim_arena_create")
	fc.out.PopReg("rsi")
	fc.out.PopReg("rdi")
	...
	// Increment counter
	fc.out.AddImmToReg("r13", 1)
	jumpOffset2 := int32(firstInitLoopLabel - (fc.eb.text.Len() + UnconditionalJumpSize))
	fc.out.JumpUnconditional(jumpOffset2)
...
// This is used within tim_arena_alloc when an arena runs out of space
// Input registers:
//
//	rdi = arena_ptr
...
func (fc *TimCompiler) generateIndividualArenaGrowth(arenaGrowJump int) int {
