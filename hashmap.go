...
// TimHashMap represents a hash map from uint64 to float64
// This is the fundamental datastructure in Tim
// All values (ints, strings, etc) are stored in such a hash map
type TimHashMap struct {
	buckets []TimHashBucket
	size    int
	count   int
}

type TimHashBucket struct {
	key      uint64
	value    float64
	occupied bool
	next     *TimHashBucket
...
// NewTimHashMap creates a new hash map with the given initial size
func NewTimHashMap(initialSize int) *TimHashMap {
	if initialSize < 16 {
		initialSize = 16
	}
	return &TimHashMap{
		buckets: make([]TimHashBucket, initialSize),
		size:    initialSize,
		count:   0,
	}
}
...
func (m *TimHashMap) hash(key uint64) uint64 {
	h := fnv.New64a()
	bytes := make([]byte, 8)
	for i := range 8 {
...
func (m *TimHashMap) Get(key uint64) (float64, bool) {
...
func (m *TimHashMap) Set(key uint64, value float64) {
	idx := m.hash(key) % uint64(m.size)
	bucket := &m.buckets[idx]
...
func (m *TimHashMap) resize() {
	oldBuckets := m.buckets
	m.size *= 2
	m.buckets = make([]TimHashBucket, m.size)
	m.count = 0
...
func (m *TimHashMap) Delete(key uint64) bool {
	idx := m.hash(key) % uint64(m.size)
	bucket := &m.buckets[idx]
...
func (m *TimHashMap) Values() []float64 {
