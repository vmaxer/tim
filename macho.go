...
func generateCodeSignature(identifier string, binaryData []byte, execSegBase, execSegLimit uint64) ([]byte, error) {
	...
	for i := range nPages {
		...
		pageEnd := min(pageStart+CS_PAGE_SIZE, len(binaryData))
...
func (eb *ExecutableBuilder) WriteMachO() error {
	...
	debug := os.Getenv("TIM_DEBUG") != ""

	if debug || VerboseMode {
		debugf("DEBUG: WriteMachO() called, text.Len()=%d, useDynamicLinking=%v, neededFunctions=%v\n",
	...
	if debug || VerboseMode {
		debugf("DEBUG: headerSize=%d, prelimLoadCmdsSize=%d, fileHeaderSize=%d\n",
	...
	// These are functions like _tim_itoa, _tim_string_concat that are in the text section
	for labelName, labelOffset := range eb.labels {
		...
		// Our labels have one underscore (_tim_itoa), but Mach-O needs two (__tim_itoa)
	...
	// Write Mach-O header
	if debug {
		debugf("DEBUG: About to write Mach-O header with NCmds=%d, SizeOfCmds=%d\n", ncmds, loadCmdsSize)
	}
	...
	if debug || VerboseMode {
		debugf("DEBUG: Mach-O header flags = 0x%08x (MH_PIE=0x%x, useDynamicLinking=%v)\n", flags, MH_PIE, eb.useDynamicLinking)
	}
	...
	if debug {
		debugf("DEBUG: Wrote Mach-O header\n")
	}
	...
	if len(bufBytes) >= 32 {
		...
		if debug || VerboseMode {
			debugf("DEBUG: Header in buffer has NCmds=%d, SizeOfCmds=%d, Flags=0x%08x\n",
	...
	if debug || VerboseMode {
		debugf("DEBUG: Before padding, buf.Len()=%d, textFileOffset=%d\n", buf.Len(), textFileOffset)
	}
	...
	if debug || VerboseMode {
		debugf("DEBUG: Writing __text at offset %d, size=%d bytes: %x\n", buf.Len(), eb.text.Len(), eb.text.Bytes())
	}
	...
	if debug || VerboseMode {
		...
		if len(finalBytes) >= 824 {
			debugf("DEBUG: Final buffer, bytes at offset 816: %x\n", finalBytes[816:824])
		}
	}
