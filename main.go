...
const versionString = "timcc 1.0.0"
...
func (eb *ExecutableBuilder) PatchPCRelocations(textAddr, rodataAddr uint64, rodataSize int) {
	debugf("DEBUG PatchPCRelocations called: %d relocations, textAddr=0x%x, rodataAddr=0x%x\n", len(eb.pcRelocations), textAddr, rodataAddr)

	// Print which symbol addresses we have RIGHT NOW
	if VerboseMode && strings.Contains(fmt.Sprintf("%v", eb.consts), "arena") {
		for name, c := range eb.consts {
			if strings.Contains(name, "arena_meta") {
				debugf("  Symbol %s has address 0x%x at start of patching\n", name, c.addr)
			}
		}
	}

	if VerboseMode {
		debugf("DEBUG: Relocations to patch:\n")
		for i, reloc := range eb.pcRelocations {
			debugf("  [%d] offset=0x%X symbol=%s\n", i, reloc.offset, reloc.symbolName)
			if i >= 10 {
	...
	for _, reloc := range eb.pcRelocations {
		...
		if labelOffset, ok := eb.labels[reloc.symbolName]; ok {
			...
			found = true
			if VerboseMode {
				debugf("DEBUG PatchPCRelocations: function %s using address 0x%x (textAddr=0x%x, offset=%d)\n",
		...
		if !found {
			if c, ok := eb.consts[reloc.symbolName]; ok {
				...
				if strings.HasPrefix(reloc.symbolName, "str_") {
					if VerboseMode {
						debugf("DEBUG PatchPCRelocations: %s using address 0x%x\n", reloc.symbolName, targetAddr)
					}
				} else if strings.Contains(reloc.symbolName, "parallel") && VerboseMode {
					debugf("DEBUG PatchPCRelocations: %s found in consts with address 0x%x\n", reloc.symbolName, targetAddr)
				} else if strings.Contains(reloc.symbolName, "arena") {
					debugf("DEBUG PatchPCRelocations: ARENA symbol %s using address 0x%x (writable=%v) at reloc offset 0x%x\n", reloc.symbolName, targetAddr, c.writable, reloc.offset)
				}
			} else if VerboseMode {
				debugf("DEBUG PatchPCRelocations: label '%s' not found in labels or consts (have %d labels)\n", reloc.symbolName, len(eb.labels))
			}
		}
...
func (eb *ExecutableBuilder) PatchCallSites(textAddr uint64) {
	...
	if VerboseMode {
		debugf("DEBUG PatchCallSites: Have %d labels, %d call patches\n", len(eb.labels), len(eb.callPatches))
	...
	for _, patch := range eb.callPatches {
		...
		if targetOffset < 0 && strings.HasSuffix(patch.targetName, "$stub") {
			...
			if VerboseMode && targetOffset >= 0 {
				debugf("DEBUG: Found internal label %s at offset %d (was looking for %s)\n", baseName, targetOffset, patch.targetName)
			}
		}
		if targetOffset < 0 {
			if VerboseMode {
...
func (eb *ExecutableBuilder) Bytes() []byte {
	...
	if eb.target.IsMachO() {
		if err := eb.WriteMachO(); err != nil {
			...
			result := eb.elf.Bytes()
			if VerboseMode {
				debugf("DEBUG Bytes(): Using Mach-O format (size=%d)\n", len(result))
				if len(result) >= 824 {
					debugf("DEBUG Bytes(): bytes at offset 816: %x\n", result[816:824])
				}
			}
			return result
		}
	...
	if eb.useDynamicLinking {
		if VerboseMode {
			debugf("DEBUG Bytes(): Using dynamic ELF, returning eb.elf only (size=%d)\n", eb.elf.Len())
		}
		return eb.elf.Bytes()
	}
	...
	if VerboseMode {
		debugf("DEBUG Bytes(): Using static ELF, concatenating sections\n")
	}
...
func (eb *ExecutableBuilder) DefineAddr(symbol string, addr uint64) {
	if c, ok := eb.consts[symbol]; ok {
		if strings.HasPrefix(symbol, "str_") || strings.HasPrefix(symbol, "lambda_") {
			if VerboseMode {
				debugf("DEBUG DefineAddr: %s set to 0x%x\n", symbol, addr)
			}
		}
		if strings.Contains(symbol, "arena") {
			oldAddr := c.addr
			debugf("DEBUG DefineAddr: ARENA symbol %s: 0x%x -> 0x%x\n", symbol, oldAddr, addr)
		}
		c.addr = addr
	} else {
		...
		if strings.HasPrefix(symbol, "lambda_") {
			if VerboseMode {
				debugf("DEBUG DefineAddr: Creating missing symbol %s with addr 0x%x\n", symbol, addr)
			}
		}
		if strings.Contains(symbol, "arena") {
			debugf("DEBUG DefineAddr: ARENA symbol %s created with addr 0x%x\n", symbol, addr)
		}
...
func (eb *ExecutableBuilder) MarkLabel(label string) {
	...
	if VerboseMode {
		debugf("DEBUG MarkLabel: %s at offset %d\n", label, offset)
	}
}
...
func (eb *ExecutableBuilder) GenerateCallInstruction(funcName string) error {
	...
	// - Internal Tim runtime functions (starting with _tim)
	// - Functions starting with double underscore (like __acrt_iob_func on Windows)
	targetName := funcName
	if eb.target.OS() == OSDarwin && strings.HasPrefix(funcName, "_") && !strings.HasPrefix(funcName, "_tim") && !strings.HasPrefix(funcName, "__") {
	...
	if !strings.HasPrefix(funcName, "_tim") && !strings.HasPrefix(funcName, "__") {
		patchName += "$stub"
	}
...
func (eb *ExecutableBuilder) EmitArenaRuntimeCode() {
	...
	// tim_arena_create(capacity) - creates an arena with given capacity
	...
	eb.MarkLabel("_tim_arena_create")
	out.PushReg("rbp")
	...
	// tim_arena_destroy(arena_ptr) - destroys an arena
	// Argument: rdi = arena_ptr
	eb.MarkLabel("_tim_arena_destroy")
	out.PushReg("rbp")
	out.MovRegToReg("rbp", "rsp")
	out.PushReg("rbx")
...
func (eb *ExecutableBuilder) patchTextInELF() {
	...
	debugf("DEBUG patchTextInELF: elfBuf size=%d, newText size=%d\n", len(elfBuf), len(newText))
	debugf("DEBUG patchTextInELF: newText first 50 bytes: %x\n", newText[:min(50, len(newText))])
	...
	if VerboseMode {
		...
		for i := 0; i <= len(newText)-3; i++ {
			if newText[i] == 0x49 && newText[i+1] == 0xFF && newText[i+2] == 0xD3 {
				debugf("DEBUG patchTextInELF: Found 49 FF D3 at offset %d (0x%x) in newText\n", i, i)
				callFound = true
				break
			}
		}
		if !callFound {
			debugf("DEBUG patchTextInELF: WARNING - 49 FF D3 (call r11) NOT FOUND in newText buffer!\n")
		}
	}
	...
	if VerboseMode {
		debugf("DEBUG patchTextInELF: textOffset=0x%x, textSize=%d\n", textOffset, textSize)
		debugf("DEBUG patchTextInELF: about to copy newText to elfBuf[0x%x:0x%x]\n", textOffset, textOffset+textSize)
	}
	...
	if textEndAligned > textReservedEnd {
		...
		debugf("DEBUG patchTextInELF: Copying %d bytes from newText to elfBuf[0x%x:0x%x]\n", textSize, textOffset, textOffset+textSize)
		copy(elfBuf[textOffset:textOffset+textSize], newText)
		debugf("DEBUG patchTextInELF: After copy, elfBuf[0x309b:0x30a3] = %x\n", elfBuf[0x309b:0x30a3])
	}

	// Get fresh reference to buffer after potential resize
	elfBuf = eb.elf.Bytes()
	if VerboseMode {
		debugf("DEBUG patchTextInELF: after copy, elfBuf[0x%x:0x%x] = %x\n",
...
func (eb *ExecutableBuilder) patchDynsymInELF(ds *DynamicSections) {
	...
	if VerboseMode {
		debugf("DEBUG patchDynsymInELF: elfBuf size=%d, newDynsym size=%d\n", len(elfBuf), len(newDynsym))
		debugf("DEBUG patchDynsymInELF: dynsymOffset=0x%x, dynsymSize=%d\n", dynsymOffset, dynsymSize)
	}

	if dynsymOffset > 0 && dynsymOffset+dynsymSize <= len(elfBuf) {
		if VerboseMode {
			debugf("DEBUG patchDynsymInELF: copying newDynsym to elfBuf[0x%x:0x%x]\n", dynsymOffset, dynsymOffset+dynsymSize)
		}
		copy(elfBuf[dynsymOffset:dynsymOffset+dynsymSize], newDynsym)
		if VerboseMode {
			debugf("DEBUG patchDynsymInELF: patched symbol table successfully\n")
		}
	} else if VerboseMode {
		debugf("DEBUG patchDynsymInELF: WARNING - invalid offset or size (offset=%d, size=%d, elfBuf=%d)\n",
...
func main() {
	...
	var codeFlag = flag.String("c", "", "execute Tim code from command line")
	...
	var singleFlag = flag.Bool("single", false, "compile single file only (don't load other .tim files from directory)")
	...
	if VerboseMode {
		debugf("DEBUG main: VerboseMode enabled\n")
	}
	...
	if *targetFlag != "" {
		...
		if autoDetectWindows {
			...
			if VerboseMode {
				debugf("DEBUG: Not auto-detecting Windows: targetExplicit=%v osExplicit=%v outputProvided=%v hasSuffix=%v\n",
	...
	if len(inputFiles) > 0 {
		...
		isTimFile := strings.HasSuffix(firstArg, ".tim")
	...
	// FALLBACK: No arguments and no -c flag - check for .tim files or show help
	if len(inputFiles) == 0 && *codeFlag == "" {
		matches, err := filepath.Glob("*.tim")
	...
	if *codeFlag != "" {
		...
		tmpFile, err := os.CreateTemp("", "tim_*.tim")
		if err != nil {
		...
		if outputFilename == defaultOutputFilename {
			writeToFilename = filepath.Join(os.TempDir(), "tim_inline")
			inlineFlagProvided = false
		}

		err = CompileTimWithOptions(tmpFilename, writeToFilename, targetPlatform, *optTimeout, VerboseMode, *depsFlag)
		if err != nil {
	...
	if len(inputFiles) > 0 {
		for _, file := range inputFiles {
			...
			// Check if this is a Tim source file
			if strings.HasSuffix(file, ".tim") {
				if VerboseMode {
					fmt.Fprintln(os.Stderr, "-> Compiling Tim source")
				}

				writeToFilename := outputFilename
				if !outputFlagProvided {
					// No output filename specified - use input filename without .tim
					writeToFilename = strings.TrimSuffix(filepath.Base(file), ".tim")
				}

				err := CompileTimWithOptions(file, writeToFilename, targetPlatform, *optTimeout, VerboseMode, *depsFlag)
				if err != nil {
					fmt.Fprintf(os.Stderr, "%v\n", err)
					os.Exit(1)
				}
