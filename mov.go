...
func (o *Out) callSymbolX86(symbol string) {
	if strings.Contains(symbol, "arena") {
		debugf("DEBUG: CallSymbol(%s) at text position %d (0x%X)\n", symbol, o.eb.text.Len(), o.eb.text.Len())
	}
	...
	if o.target.OS() == OSWindows {
		...
		if envBool("DEBUG") && (symbol == "_tim_arena_ensure_capacity" || symbol == "malloc$stub") {
			debugf("DEBUG mov.go: CallSymbol(%s) Windows: FF 15 at %d, callPos=%d\n",
		...
		if envBool("DEBUG") && (symbol == "_tim_arena_ensure_capacity" || symbol == "malloc$stub") {
			debugf("DEBUG mov.go: CallSymbol(%s): E8 at %d, callPos=%d\n",
				symbol, posBeforeCall, callPos)
		}
