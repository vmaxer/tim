...
func (eb *ExecutableBuilder) WritePE(outputPath string) error {
	...
	// Standard C runtime functions needed by Tim programs
...
func BuildPEImportData(libraries map[string][]string, idataRVA uint32) ([]byte, map[string]uint32, error) {
	...
	for i := range libsData {
		...
		if VerboseMode {
			debugf("DEBUG: %s hintsOffset = 0x%x (currentOffset before hints)\n", libsData[i].name, currentOffset)
		}
	...
	for libIdx, ld := range libsData {
		...
		if VerboseMode {
			debugf("DEBUG: Writing ILT for %s, starting hintOffset=0x%x\n", ld.name, hintOffset)
		}
		...
		if VerboseMode {
			debugf("DEBUG: Finished ILT for %s\n", ld.name)
		}
		...
		if VerboseMode {
			debugf("DEBUG: Writing IAT for %s (offset=0x%x, iatBase=0x%x, functions=%v)\n", ld.name, ld.iatOffset, iatBase, ld.functions)
			debugf("DEBUG: Starting hintOffset=0x%x\n", hintOffset)
		}
		...
		for funcIndex, funcName := range ld.functions {
			...
			if funcName == "ExitProcess" || funcName == "malloc" {
				debugf("DEBUG:   %s -> IAT RVA=0x%X (iatBase=0x%X + %d*8), hint RVA=0x%X\n",
			...
			if funcName == "ExitProcess" || funcName == "malloc" {
				debugf("DEBUG:   Writing %s IAT: hintRVA=0x%X, buf position=0x%X\n", funcName, hintRVA, buf.Len()-8)
			}
	...
	for libIdx, ld := range libsData {
		if VerboseMode {
			debugf("DEBUG: Writing hints for %s starting at buf offset 0x%x\n", ld.name, buf.Len())
		}
		for _, funcName := range ld.functions {
			...
			if VerboseMode && libIdx == 0 && (funcName == "SDL_RenderTexture" || funcName == "SDL_RenderPresent" || funcName == "SDL_RenderClear") {
				debugf("DEBUG:   %s: wrote %d bytes (buf %d -> %d)\n", funcName, afterLen-beforeLen, beforeLen, afterLen)
			}
		}
		if VerboseMode {
			debugf("DEBUG: Finished hints for %s at buf offset 0x%x\n", ld.name, buf.Len())
		}
	}
