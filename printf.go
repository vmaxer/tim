...
func EmitPrintfRuntimeX86_64(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		debugf("DEBUG: Emitting _tim_printf for x86-64\n")
	}

	eb.MarkLabel("_tim_printf")
	...
	// rdi = format string pointer (Tim map)
	...
	if VerboseMode {
		debugf("DEBUG: _tim_printf emitted successfully\n")
	}
...
func EmitPrintfRuntimeARM64(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		debugf("DEBUG: Emitting _tim_printf for ARM64\n")
	}
	...
	if VerboseMode {
		debugf("DEBUG: ARM64 _tim_printf stub emitted\n")
	}
...
func EmitPrintfRuntimeRISCV64(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		debugf("DEBUG: Emitting _tim_printf for RISC-V 64\n")
	}
	...
	if VerboseMode {
		debugf("DEBUG: RISC-V _tim_printf stub emitted\n")
	}
