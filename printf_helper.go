...
func (p *PrintfCodeGen) PatchJump(jumpPos int, targetPos int, jumpInstrSize int) {
	...
	if VerboseMode {
		debugf("DEBUG PatchJump: jumpPos=%d, targetPos=%d, offset=%d\n",
...
// Signature: _tim_float_to_string(xmm0: float64, rdi: buffer_ptr) -> (rsi: str_start, rdx: length)
func EmitFloatToStringRuntime(eb *ExecutableBuilder, out *Out) error {
	if VerboseMode {
		debugf("DEBUG: Emitting _tim_float_to_string runtime function\n")
	}

	eb.MarkLabel("_tim_float_to_string")
	...
	if VerboseMode {
		debugf("DEBUG: _tim_float_to_string emitted successfully\n")
	}
