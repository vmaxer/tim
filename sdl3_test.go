...
`
	...
	tmpFile, err := os.CreateTemp("", "sdl3_windows_test_*.tim")
	if err != nil {
	...
	err = CompileTim(tmpPath, outputPath, platform)
	if err != nil {
...
func TestSDL3FloatFFI(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("Skipping Linux SDL3 test on non-Linux platform")
	}
...
x = 0.5
sin_d = sdl.SDL_sin(x)
sin_f = sdl.SDL_sinf(x)
printf("sin_d=%.6f sin_f=%.6f\n", sin_d, sin_f)
`

	result := compileAndRun(t, source)
	if !strings.Contains(result, "sin_d=0.479426 sin_f=0.479426") {
		t.Errorf("expected SDL_sin and SDL_sinf to match, got: %s", result)
	}
}
...
func TestSDL3ExampleCompiles(t *testing.T) {
	// Just verify sdl3example.tim compiles for both targets
	data, err := os.ReadFile("sdl3example.tim")
	if err != nil {
		t.Skipf("sdl3example.tim not found: %v", err)
	}
	...
	if runtime.GOOS == "linux" {
		t.Run("Linux", func(t *testing.T) {
			...
			t.Logf("Successfully compiled sdl3example.tim for Linux: %d bytes", info.Size())
		})
	}
	...
	t.Run("Windows", func(t *testing.T) {
		...
		t.Logf("Successfully compiled sdl3example.tim for Windows: %d bytes", len(data))
	})
}
