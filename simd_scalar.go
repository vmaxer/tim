...
// MinsdXmm - Minimum Scalar Double (SSE2), dst = min(dst, src)
func (o *Out) MinsdXmm(dst, src string) {
	switch o.target.Arch() {
	case ArchX86_64:
		o.minsdX86(dst, src)
	}
...
func (o *Out) minsdX86(dst, src string) {
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "minsd %s, %s: ", dst, src)
	}
...
// MaxsdXmm - Maximum Scalar Double (SSE2), dst = max(dst, src)
func (o *Out) MaxsdXmm(dst, src string) {
	switch o.target.Arch() {
	case ArchX86_64:
		o.maxsdX86(dst, src)
	}
...
func (o *Out) maxsdX86(dst, src string) {
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "maxsd %s, %s: ", dst, src)
	}
...
// Fldlg2 pushes log10(2) onto FPU stack
func (o *Out) Fldlg2() {
	if VerboseMode {
		fmt.Fprintf(os.Stderr, "fldlg2: ")
	}
	o.Write(0xD9)
	o.Write(0xEC)
	if VerboseMode {
